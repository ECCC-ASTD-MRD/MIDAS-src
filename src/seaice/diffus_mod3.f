...
!-----------------------------------------------------------------------
      ...
      public :: sudiff, fin_diff, Csqrt, Csqrtadj,
     &          diffus_data, test_diff
...
!     Number of samples in the estimation of the normalization factors by randomization.
      ...
      namelist /diffusion_nml/ stab, nsamp, limplicit

!*************************************************************************

      contains

      subroutine sudiff(paramlist, diff_norm_fact, corr_len, diff)
...
!     Export the length scale factor
      ...
      if (.not. allocated(cosyhalf))  allocate(cosyhalf(NJ))
      if (.not. allocated(cosyinv))   allocate(cosyinv(NJ))
      if (.not. allocated(cosyinvsq)) allocate(cosyinvsq(NJ))
      if (.not. allocated(Winv))      allocate(Winv(NI,NJ))
      if (.not. allocated(Wsqrt))     allocate(Wsqrt(NI,NJ))
      if (.not. allocated(Winvsqrt))  allocate(Winvsqrt(NI,NJ))
      if (.not. allocated(mhalfx))    allocate(mhalfx(NI-1,NJ))
      if (.not. allocated(mhalfy))    allocate(mhalfy(NI,NJ-1))

      allocate(diff%khalfx(NI-1,NJ), diff%khalfy(NI,NJ-1))
      allocate(diff%Lambda(NI,NJ))

      allocate(diff%diff1x_ap(NI,NJ))
      allocate(diff%diff1x_bp_inv(NI,NJ))
      allocate(diff%diff1x_c(NI,NJ))
      allocate(diff%diff1y_ap(NJ,NI))
      allocate(diff%diff1y_bp_inv(NJ,NI))
      allocate(diff%diff1y_c(NJ,NI))
...
! compute the LU decomposition for the implicit 1D diffusion
      diff%diff1x_ap(:,:)=0.0d0
      diff%diff1x_bp_inv(:,:)=0.0d0      
      diff%diff1x_c(:,:)=0.0d0
      diff%diff1y_ap(:,:)=0.0d0
      diff%diff1y_bp_inv(:,:)=0.0d0      
      diff%diff1y_c(:,:)=0.0d0
...
!$OMP DO PRIVATE(j,i,a,b)
      do j=2,NJ-1
         i=2
         diff%diff1x_bp_inv(i,j)= 1.0d0/(1.0d0 + 
     +        diff%dt*cosyinvsq(j)*(mhalfx(i,j)*diff%khalfx(i,j) + 
     +        mhalfx(i-1,j)*diff%khalfx(i-1,j))/(dlon*dlon))
         do i=3,NI-1
            ! elements of the tri-diagonal coefficient matrix
            a=   - diff%dt*cosyinvsq(j)*mhalfx(i-1,j)*diff%khalfx(i-1,j)
     +                /(dlon*dlon)
            b=    1+diff%dt*cosyinvsq(j)*(mhalfx(i,j)*diff%khalfx(i,j) +
     +                mhalfx(i-1,j)*diff%khalfx(i-1,j))/(dlon*dlon)
            diff%diff1x_c(i,j) = -diff%dt*cosyinvsq(j)*mhalfx(i,j)
     +                      *diff%khalfx(i,j)
     +                /(dlon*dlon)
            diff%diff1x_ap(i,j)=a*diff%diff1x_bp_inv(i-1,j)
            diff%diff1x_bp_inv(i,j)=1.0d0/(b-a*diff%diff1x_c(i-1,j)
     +                         *diff%diff1x_bp_inv(i-1,j))
         enddo
      enddo
!$OMP END DO
!$OMP END PARALLEL
...
!$OMP DO PRIVATE(j,i,a,b)
      do i=2,NI-1
         j=2
         diff%diff1y_bp_inv(j,i)= 1.0d0/(1.0d0 + 
     +     diff%dt*cosyinvsq(j)*(cosyhalf(j)*mhalfy(i,j)
     +     *diff%khalfy(i,j) + 
     +     cosyhalf(j-1)*mhalfy(i,j-1)*diff%khalfy(i,j-1))/(dlat*dlat))
         do j=3,NJ-1
            ! elements of the tri-diagonal coefficient matrix
            a= - diff%dt*cosyinv(j)*cosyhalf(j-1)*
     +           mhalfy(i,j-1)*diff%khalfy(i,j-1)/(dlat*dlat)
            b= 1 + diff%dt*cosyinv(j)*(cosyhalf(j)*mhalfy(i,j)*
     +         diff%khalfy(i,j) + 
     +         cosyhalf(j-1)*mhalfy(i,j-1)*diff%khalfy(i,j-1))
     +         /(dlat*dlat)
            diff%diff1y_c(j,i)= - diff%dt*cosyinv(j)*cosyhalf(j)*
     +           mhalfy(i,j)*diff%khalfy(i,j)/(dlat*dlat)
            diff%diff1y_ap(j,i)=a*diff%diff1y_bp_inv(j-1,i)
            diff%diff1y_bp_inv(j,i)=1.0d0/(b-a*diff%diff1y_c(j-1,i)
     +                         *diff%diff1y_bp_inv(j-1,i))
         enddo
      enddo
!$OMP END DO
!$OMP END PARALLEL
...
!-----------------------------------------------------------------------
      ...
      implicit none
      type(diffus_data), intent(inout) :: diff

      if (allocated(cosyhalf))  deallocate(cosyhalf)
      if (allocated(cosyinv))   deallocate(cosyinv)
      if (allocated(cosyinvsq)) deallocate(cosyinvsq)
      if (allocated(Winv))      deallocate(Winv)
      if (allocated(Wsqrt))     deallocate(Wsqrt)
      if (allocated(Winvsqrt))  deallocate(Winvsqrt)
      if (allocated(mhalfx))    deallocate(mhalfx)
      if (allocated(mhalfy))    deallocate(mhalfy)

      if (allocated(diff%diff1x_ap)) deallocate(diff%diff1x_ap)
      if (allocated(diff%diff1x_bp_inv)) deallocate(diff%diff1x_bp_inv)
      if (allocated(diff%diff1x_c))  deallocate(diff%diff1x_c)
      if (allocated(diff%diff1y_ap)) deallocate(diff%diff1y_ap)
      if (allocated(diff%diff1y_bp_inv)) deallocate(diff%diff1y_bp_inv)
      if (allocated(diff%diff1y_c))  deallocate(diff%diff1y_c)

      if (allocated(diff%khalfx)) deallocate(diff%khalfx)
      if (allocated(diff%khalfy)) deallocate(diff%khalfy)
      if (allocated(diff%Lambda)) deallocate(diff%Lambda)
...
!$OMP DO PRIVATE(j,i,dp)
         do j=2,NJ-1
            ...
            dp(i)=xlast(i,j)
            do i=3,NI-1
               dp(i)=xlast(i,j) - diff%diff1x_ap(i,j)*dp(i-1)
            enddo
            i=NI-1
            xout(i,j)=dp(i)*diff%diff1x_bp_inv(i,j)
            do i=NI-2,2,-1
               xout(i,j)=(dp(i)-diff%diff1x_c(i,j)*xout(i+1,j))
     +                   *diff%diff1x_bp_inv(i,j)
            enddo
!            if(t.lt.numt2) then
!               do i=1,NI
!                  xlast(i,j) = xout(i,j)
...
!$OMP DO PRIVATE(j,i,dp)
         do i=2,NI-1
            ...
            dp(j)=xlast(j,i)
            do j=3,NJ-1
               dp(j)=xlast(j,i) - diff%diff1y_ap(j,i)*dp(j-1)
            enddo
            j=NJ-1
            xout(i,j)=dp(j)*diff%diff1y_bp_inv(j,i)
            do j=NJ-2,2,-1
               xout(i,j)=(dp(j)-diff%diff1y_c(j,i)*xout(i,j+1))
     +                   *diff%diff1y_bp_inv(j,i)
            enddo 
!            if(t.lt.numt2) then
!               do j=1,NJ
!                  xlast(j,i) = xout(i,j)
