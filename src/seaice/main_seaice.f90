...
!! *Origin *: ensmanip/main_ensmanip.ftn90
!!
!--------------------------------------------------------------------------
program main_seaIce
  ...
  if(ierr.ne.0) call utl_abort('main_seaIce: Error reading namelist')
           ...
           do jlat = 1, nlats
              ...
              if(oneobs_lats(jlat) >= statevector%myLatBeg .and. oneobs_lats(jlat) <= statevector%myLatEnd .and.  &
                 ...
                 if(vnl_varLevelFromVarname(vnl_varNameList(jvar)) == 'SF') then
                    ...
                    call utl_abort('main_seaIce: Expecting only surface field')
                 endif
              endif
  ...
  !
  if( mpi_myid == 0 ) write(*,*) ' --------------------------------'
  if( mpi_myid == 0 ) write(*,*) ' MAIN_SEAICE ENDS'
  if( mpi_myid == 0 ) write(*,*) ' --------------------------------'

end program main_seaIce
