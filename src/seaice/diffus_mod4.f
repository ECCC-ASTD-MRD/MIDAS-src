...
!-----------------------------------------------------------------------
      ...
      public :: sudiff, fin_diff, Csqrt, Csqrtadj, diffusion, corr_diff,
     &          diffus_data, loc_diff, test_diff, ni, nj
...
!*************************************************************************
      ...
      subroutine sudiff(paramlist, diff_norm_fact, corr_len, mask,
     $                  ni_i, nj_i, diff)
...
!     diff_norm_fact is the name of the RPN format file for the normalization factors.
      ...
      integer, intent(in)                :: ni_i, nj_i
      real, dimension(ni_i,nj_i), intent(in) :: mask
      type(diffus_data), intent(out)     :: diff

      !> Grid parameters
      real, parameter :: Grd_dx     = 0.045
      real, parameter :: Grd_dy     = 0.045
      real, parameter :: Grd_iref   = 800
      real, parameter :: Grd_jref   = 478
      real, parameter :: Grd_latr   = -0.0225
      real, parameter :: Grd_lonr   = 179.9775

      !> Constants
      real, parameter :: earth_radius = 6371.22
      real :: deg2rad
!     Local Variables
      ...
      double precision, dimension(:,:), allocatable :: Lcorr
      double precision, dimension(:,:), allocatable :: kappa
      double precision, dimension(:), allocatable   :: latr
      double precision, dimension(:,:), allocatable :: W
      double precision, dimension(:,:), allocatable :: m
      double precision, dimension(:,:), allocatable :: xin
...
!     Export the length scale factor
      ...
      allocate(Lcorr(NI,NJ), kappa(NI,NJ), latr(NJ))
      allocate(W(NI,NJ), m(NI,NJ), xin(NI,NJ))
...
!     minimum grid spacing over the domain (radians, but with cos(latr))
      ...
      write(*,*) 'maxL: ', maxL, ', corrlen: ', diff%corr_len
      write(*,*) 'earth rad: ', earth_radius
...
!$OMP DO PRIVATE(j,i,a,b)
     ...
     +                mhalfx(i-1,j)*diff%khalfx(i-1,j))/(dlon*dlon)
            diff1x_c(i,j) = -diff%dt*cosyinvsq(j)*mhalfx(i,j)
     +                      *diff%khalfx(i,j)
     +                /(dlon*dlon)
...
!$OMP DO PRIVATE(j,i,a,b)
     ...
     +         /(dlat*dlat)
            diff1y_c(j,i)= - diff%dt*cosyinv(j)*cosyhalf(j)*
     ...
     $           'normalization for diffusion...'
            ...
            do isamp=1,nsamp
               ...
               if(limplicit) then
                 ...
                 call diffusion_explicit(xin,xin,diff)
               endif
...
!**********************************************************
      ...
      subroutine diffusion_explicit(xout,xin,diff)
...
!     this is the C^1/2 required for the forward model: Csqrt = Lambda * Diffuse * W^-1/2
      ...
      if (limplicit) then
         ...
         call diffusion_explicit(xout,xout,diff)
      endif
