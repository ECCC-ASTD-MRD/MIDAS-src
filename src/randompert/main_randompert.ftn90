...
program main_randomPert
  !
  !**   main_randomPert  - Program to generate an ensemble of random perturbations
  !
  use mpi_mod
  use mpivar_mod
  ...
  NAMELIST /NAMENKF/nens, seed, date, remove_mean, smoothVariances, write_mpi

  write(*,'(/,' //  &
        '3(" *****************"),/,' //                   &
        '14x,"-- START OF MAIN_RANDOMPERT           --",/,' //   &
  ...
  if(ierr.ne.0) call utl_abort('main_randomPert: Error reading namelist')
...
!$OMP END PARALLEL DO
    ...
    if( mpi_myid == 0 ) write(*,*)'MAIN_RANDOMPERT: PROCESSING clfiname= ', clfiname

    if( write_mpi ) then
      call gsv_writeToFileMPI(statevector, clfiname, 'RANDOM_PERT',      & ! IN
                              HUcontainsLQ=.true., unitConversion=.true.)  ! IN
    else
      call gsv_writeToFile(statevector, clfiname, 'RANDOM_PERT',      & ! IN
                           HUcontainsLQ=.true., unitConversion=.true.)  ! IN
    end if
  ...
  !
  if( mpi_myid == 0 ) write(*,*) ' --------------------------------'
  if( mpi_myid == 0 ) write(*,*) ' MAIN_RANDOMPERT ENDS'
  if( mpi_myid == 0 ) write(*,*) ' --------------------------------'
