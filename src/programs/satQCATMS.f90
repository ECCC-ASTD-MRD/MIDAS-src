...
program midas_satQCATMS
  ...
  integer(kind=int_def)  :: indice, indice1, indice2, indice3, indice4, kk
  ...
  real, dimension(mxval*mxnt)          :: zdata,ztb, biasCorr
  ...
  integer :: nulnam
  namelist /nambgck/ debug, sp_adj_tb, modlsqtt, useUnbiasedObsForClw
  ...
  if ( sp_adj_tb ) then
    ...
    write(*,*) 'midas_satQCATMS: empty bcor file is used in the unitTest. Modifying before continuing.'
    call abort()
  endif
  ...
  REPORTS: do k = 1, compteur
    ...
    IF ( .not. resume_report ) THEN 
      ...
      if ( ANY(grossrej) ) then
        write(*,*) ' GROSS_VALUE_CHECK has detected bad Tb data. Number of affected locations = ', COUNT(grossrej)
      ...
      !if ( lutb ) then
      !  write(*,*) ' Number of Tb data = zmisg after QC_DATA = ', COUNT(ztb == zmisg)
      ...
      if ( COUNT(lqc) == nt*nchan ) then
        write(*,*) ' QC_DATA has detected a problem with data in this report!'
      ...
      if( any(alloc_status /= 0) ) then
        write(*,*) ' midas_satQCATMS: Memory deallocation error '
        call abort()
      endif
  ...
  contains
    ...
    subroutine GetData
      ...
      indice4 = BURP_Find_Element(Block_in,12233,IOSTAT=error)
      ...
      if ( indice1 > 0 .and. indice2 > 0 .and. indice3 > 0 .and. indice4 > 0 ) then
        ...
        do kk = 1, my_nt
          do j = 1, my_nval
            ...
            biasCorr(ipos)= BURP_Get_Rval(Block_in,indice4,j,kk,error)
          end do
        end do
      else
    ...
    subroutine WriteBlocks
      ...
      BLOCKS: do
        ...
        !  DATA block
        elseif (btyp == 9248 .or. btyp ==9264) then 
  ...
  !          if ( debug ) Call BURP_TO_STDOUT(Block_in, CONVERT =.FALSE.)
            ...
            Call BURP_Write_Block(Rpt_out,BLOCK=Block_in,CONVERT_BLOCK=.TRUE.,IOSTAT=error)
            if (error /= burp_noerr)  call handle_error() 
          endif

        ! FLAG block
        elseif (btyp == 15392 .or. btyp == 15408) then 
    ...
    subroutine land_ice_mask_atms(mglg_file,npts,zlat,zlon,zlq,ztt,waterobs)
      ...
      if( any(alloc_status /= 0) ) then
        write(*,*) ' LAND_ICE_MASK_ATMS: Memory allocation error '
        call abort()
      endif
      ...
      if ( key <  0 ) then
        write(*,*) ' LAND_ICE_MASK_ATMS: The MG field is MISSING '
        call abort()
      end if
      ...
      if ( key <  0 ) then
        ...
        if ( key <  0 ) then
          write(*,*) ' LAND_ICE_MASK_ATMS: No ice (LG or GL) fields found. Aborting! '
          call abort()
        else
          !write(*,*) ' LAND_ICE_MASK_ATMS: The GL field was found and will be used.'
        endif
      else
        llg=.true.
      endif
      ...
      if( any(alloc_status /= 0) ) then
        write(*,*) ' LAND_ICE_MASK_ATMS: Memory deallocation error '
        call abort()
      endif
      ier = fstfrm(iungeo)
      ier = fclos(iungeo)
    ...
    subroutine qc_data(zenith,ilq,itt,zlat,zlon,ztb,scanpos,stnid,nval,nt,lqc, &
      ...
      write(*,*) '============================================================================'
      write(*,*) 'QC_DATA: Processing data box: Stnid, lat, lon = ', stnid, blat, blon
      write(*,*) ' '
      ...
      write(*,*) 'QC_DATA: Total number of data processed in this box = ', nt*nchan
