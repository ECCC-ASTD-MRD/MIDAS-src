...
!--------------------------------------------------------------------------
    ...
    subroutine oop_Hto(obs_ass_val)
      ...
      if (min_nsim == 1) then
         ...
         if ( trim(obsoperMode) == 'bgckIR') then
           call tovs_fill_profiles(columng,obsSpaceData,datestamp,filt_rlimlvhu,.true.)
         else
           call tovs_fill_profiles(columng,obsSpaceData,datestamp,filt_rlimlvhu,.false.)
         end if
      endif
      ...
      !
      if ( trim(obsoperMode) == 'bgckIR') then
        call tovs_fill_profiles_tl(column,columng,.true.)
      else
        call tovs_fill_profiles_tl(column,columng,.false.)
      end if

      !     3.   Compute radiance
      !     .    ----------------
      !
      call tovs_rttov_tl(obsSpaceData,obs_ass_val)


      call oop_storeHdx_radiances(obsSpaceData)
  ...
  CONTAINS
    ...
    subroutine oop_HTto
      ...
      !     .    ----------------------------------
      !
      call oop_get_radiance_ad(obsSpaceData)

      !     2.   Adjoint of computing radiance
      !     .    -----------------------------
      !
      call tovs_rttov_ad(obsSpaceData)


      !     3.   Adjoint of preparation of atmospheric profiles
      !     .    ----------------------------------------------
      !
      call tovs_fill_profiles_ad(column,columng)
  ...
  subroutine oop_get_radiance_ad(obsSpaceData)
    !
...
9203      format(' tovs_calc_jo_ad: error with indxtovs')
       ...
       BODY: do 
          ...
          if ( obs_bodyElem_i(obsSpaceData,OBS_ASS,index_body).eq.1 ) then
             ichn = nint(obs_bodyElem_r(obsSpaceData,OBS_PPP,index_body))
...
9202            format(' tovs_calc_jo_ad: error with channel number')
  ...
  subroutine oop_storeHdx_radiances(obsSpaceData)
    !
