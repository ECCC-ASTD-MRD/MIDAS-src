...
contains 
  ...
  subroutine slp_radar_getlatlonHRfromRange(lat, lon, rele, rzam, ralt, r_radar, latSlant,lonSlant, h_radar,d_radar)
    ...
    latSlant = asin( sin(lat)*cos(d_radar/ec_wgs_R2) + cos(lat)*sin(d_radar/ec_wgs_R2)*cos(rzam))
    lonSlant = lon + atan2(sin(rzam)*sin(d_radar/ec_wgs_R2)*cos(lat), cos(d_radar/ec_wgs_R2)-sin(lat)*sin(latSlant))
  ...
  subroutine slp_radar_getRangefromH(h_radar, ralt, rele, r_radar)
    ...
    ! ec_wgs_R2 = 6371007.1809
    a = 1.
    b = 2.*(ec_wgs_R2+ralt)*sin(rele)
    c = -(ec_wgs_R2+h_radar)**2+(ec_wgs_R2+ralt)**2.
