...
!pik 8-2014   STATUS=IBSET(STATUS,BIT_STATUS)
  ...
  subroutine brpr_readBurp(obsdat,familytype,brp_file,filenumb)
    ...
    integer                :: date2,time2,time_sonde
                ...
                DO  il=1,nbele
                  ...
                  SELECT CASE(iele)
                    ...
                    CASE(5042)
                      ...
                      ICHAN= int(BURP_Get_RVAL(Block_in,NELE_IND = il, NVAL_IND = j, NT_IND = k, IOSTAT = error))
                    ...
                    CASE(25142)
                      ...
                      INORM= int(BURP_Get_RVAL(Block_in,NELE_IND = il, NVAL_IND = j, NT_IND = k, IOSTAT = error))
          ...
          IF  (HIRES ) THEN  ! LAT LON TIME IN DATA BLOCK (e.g. UA family)
            ...
            IF ( allocated(obsvalue_sfc) ) THEN
              ...
              IF ( HIRES_SFC) THEN
                ...
                IF ( XLON  < 0. ) XLON  = 360. + XLON
                kstamp2 = tim_yyyymmddhhToDatestamp(YMD_DATE,HM*10000)
                XLAT=XLAT*MPC_RADIANS_PER_DEGREE_R8
                XLON=XLON*MPC_RADIANS_PER_DEGREE_R8
...
! Special test for GB-GPS
    ...
    SELECT CASE(FAMTYP)
      CASE ( 'UA' , 'SW' , 'AI')
        VCO=obs_vcoPressure ! PRESSURE COORD
      CASE ( 'SF' , 'SC', 'PR', 'RO', 'GP', 'AL' )
        VCO=obs_vcoHeight ! HEIGHT COORD
      CASE ( 'TO'  )
        VCO=obs_vcoChannel ! CHANNEL NUMBER
              ...
              DO il = 1, NELE
                 if ( .not. utl_isEqual(obsvalue(il,1), MPC_missingValue_R4) ) then
                    if (liste_ele(il) == 15198.OR.liste_ele(il) == 15009.OR. &
                        ...
                        VCO=obs_vcoChemColumn  ! Assumes this is a total column measurement
                        exit
                    end if
                 end if
              END DO
