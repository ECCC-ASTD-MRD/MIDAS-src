...
contains
  ...
  subroutine epp_postProcess(ensembleTrl, ensembleAnl,                   &
    ...
    type(struct_gsv)          :: stateVectorMeanAnl, stateVectorMeanAnl4D, stateVectorMeanAnlRaw, stateVectorMeanTrl
    type(struct_gsv)          :: stateVectorCtrlTrl
    type(struct_gsv)          :: stateVectorMeanInc, stateVectorMeanInc4D
    ...
    if (tim_nstepobs == tim_nstepobsinc .or. &
        tim_nstepobs == 1 .or. &
        tim_nstepobsinc == 1) then
      write(*,*) 'epp_postProcess: nstepobs, nstepobsinc = ', tim_nstepobs, tim_nstepobsinc
    else
    ...
    if (ens_isAllocated(ensembleAnl)) then
      ...
      if (any(weightRecenter(:) >= 0.0d0) .or. useOptionTableRecenter) then
        ...
        call epp_hybridRecentering(ensembleAnl, stateVectorMeanAnl4D, &
                                   weightRecenter, weightRecenterLand, &
    ...
    if (ens_isAllocated(ensembleTrl)) then
      ...
      call epp_printRmsStats(stateVectorStdDevTrl, outFileName, elapsed = 0.0D0, ftype = 'F', &
                             nEns = nEns, writeAsciiRmsStats = writeAsciiRmsStats)
      call utl_tmg_stop(5)
    ...
    if (ens_isAllocated(ensembleAnl)) then
      ...
      call gio_writeToFile(stateVectorStdDevAnl, outFileName, etiket_anlrms,  &
                           ...
                           containsFullField_opt = .false.)
      outFileName = trim(outFileName) // '_ascii'
      call epp_printRmsStats(stateVectorStdDevAnl, outFileName, elapsed = 0.0D0, ftype = 'A', &
                             nEns = nEns, writeAsciiRmsStats = writeAsciiRmsStats)
      ...
      if (writeRawAnalStats) then
        ...
        call gio_writeToFile(stateVectorStdDevAnlRaw, outFileName, etiket_anlrms_raw, &
                             ...
                             containsFullField_opt=.false.)
        outFileName = trim(outFileName) // '_ascii'
        call epp_printRmsStats(stateVectorStdDevAnlRaw, outFileName, elapsed = 0.0D0, ftype = 'A', &
                               nEns = nEns, writeAsciiRmsStats = writeAsciiRmsStats)
      ...
      if (alphaRandomPert > 0.0D0) then
        ...
        call gio_writeToFile(stateVectorStdDevAnlPert, outFileName, etiket_anlrmspert, &
                             ...
                             containsFullField_opt = .false.)
        outFileName = trim(outFileName) // '_ascii'
        call epp_printRmsStats(stateVectorStdDevAnlPert, outFileName, elapsed = 0.0D0, ftype = 'P', &
                               nEns = nEns, writeAsciiRmsStats = writeAsciiRmsStats)
      end if
      call utl_tmg_stop(5)
  ...
  subroutine epp_addRandomPert(ensemble, stateVectorRefState, alphaRandomPert, &
    ...
    call gsv_allocate(statevectorP0Ref, 1, hco_ens, vco_randomPert, &
                      dateStamp_opt=tim_getDateStamp(), mpi_local_opt=.true., &
                      hInterpolateDegree_opt='LINEAR', &
                      varNames_opt=(/'P0'/))
  ...
  !-----------------------------------------------------------------
  subroutine epp_hybridRecentering(ensembleAnl, stateVectorMeanAnl4D, &
                                   weightRecenter, weightRecenterLand, &
                                   useOptionTableRecenter, numMembersToRecenter)
    ...
    ! Arguments:
    type(struct_ens), intent(inout) :: ensembleAnl
    type(struct_gsv), intent(inout) :: stateVectorMeanAnl4D
    ...
    if (gsv_isAllocated(stateVectorMeanAnl4D)) then
      ...
      call gsv_deallocate(stateVectorRecenterAnl4D)
      call gsv_deallocate(stateVectorEnsMean4D)
    end if
  ...
  ! epp_printRmsStats
  !-----------------------------------------------------------------
  subroutine epp_printRmsStats(stateVectorStdDev,fileName,elapsed,ftype,nEns,writeAsciiRmsStats)
    !
    ...
    logical,          intent(in) :: writeAsciiRmsStats ! Do write or not the files (if '.false.', exit immediatly)
    ...
    if (.not. writeAsciiRmsStats) then
      write(*,*) 'epp_printRmsStats: ''writeAsciiRmsStats'' is ''.false.'' so don''t write ' // trim(fileName)
      return
    end if
      ...
      do latIndex = stateVectorStdDev%myLatBeg, stateVectorStdDev%myLatEnd
        do lonIndex = stateVectorStdDev%myLonBeg, stateVectorStdDev%myLonEnd
          rmsvalue(varLevIndex) = rmsvalue(varLevIndex) +  &
               (stdDev_ptr_r4(lonIndex,latIndex,varLevIndex)**2) *  &
    ...
    do varLevIndex = 1, numVarLev
      if ( (nomvar_v(varLevIndex) == 'UU') .or. (nomvar_v(varLevIndex) == 'VV') ) then
        ...
        scaleFactor(varLevIndex) = MPC_MBAR_PER_PA_R8
      else
        scaleFactor(varLevIndex) = 1.0d0
      end if
      rmsvalue(varLevIndex) = scaleFactor(varLevIndex) * rmsvalue(varLevIndex)
    end do
    ...
    if (mmpi_myid == 0) then
      ...
      if ( (nomvar_v(1) == 'UU') .and. (nomvar_v(1+nLev_M) == 'VV') ) then
        do levIndex = 1, nLev_M
          ...
          varLevIndexVV = levIndex + nLev_M
          write(nulFile,100) elapsed, ftype, nEns, nomvar_v(varLevIndexUU), &
                             pressureOrHeightOrDepth(varLevIndexUU), rmsvalue(varLevIndexUU)
          write(nulFile,100) elapsed, ftype, nEns, nomvar_v(varLevIndexVV), &
