...
module obsErrors_mod
  ...
  public :: oer_setInterchanCorr, oer_inflateErrAllsky, oer_estimateObsErrorTOVS
 ...
 ! Arrays for QC purpose
  ...
  logical :: mwAllskyTtInflateByOmp     ! choose to inflate all sky TT radiance errors by an amount related to O-P
...
! Initialization of the correlation matrices
  ...
  subroutine oer_setObsErrors(obsSpaceData, obserrorMode_in, useTovsUtil_opt)
    ...
    amsuaClearCldPredThresh(:) = 0.03D0
          ...
          if (readOldSymmetricObsErrFile) then
            ...
            if (CINSTR == "AMSUA") then
              ...
              if (amsuaChannelNum >= 1 .and.  amsuaChannelNum <= 5) then
                clearCldPredThreshInput(ICHNIN2(JI),JL) = amsuaClearCldPredThresh(amsuaChannelNum)
              end if
            end if
          else
            read(ILUTOV2,*) ICHNIN2(JI), &
    ...
    if (chanIsAllskyTt) then
      ...
      if (mwAllskyTtInflateByOmp .and. &
          ((clwObs - clearCldPredThresh(channelNumber_withOffset,sensorIndex)) * &
           (clwFG  - clearCldPredThresh(channelNumber_withOffset,sensorIndex)) < 0) .and. &
      ...
      if (mwAllskyHuInflateByOmp .and. &
          ((siObs - clearCldPredThresh(channelNumber_withOffset,sensorIndex)) * &
           (siFG  - clearCldPredThresh(channelNumber_withOffset,sensorIndex)) < 0) .and. &
...
11  CLOSE(UNIT=NULSTAT)
  ...
  !--------------------------------------------------------------------------
  ! oer_estimateObsErrorTOVS
  !--------------------------------------------------------------------------
  subroutine oer_estimateObsErrorTOVS(obsSpaceData, obsErrStdev)
    !
    ...
    real(8),              intent(inout) :: obsErrStdev(:,:) ! Estimated obs error stdev.   

    ! Locals: 
    integer              :: tovsIndex
    integer              :: headerIndex, bodyIndex, sensorIdIndex, obsIndex
    integer              :: idata, idatend, idatyp, count, channelNumber, channelIndex
    real(8), allocatable :: obsErrSum(:,:), meanObsErr(:,:), estObsERR(:,:,:), residualSqrd(:,:)
    
    integer, allocatable :: dataCountPerChan(:,:)

    write(*,*) 'ZQ_tvs_maxChannelNumber', tvs_maxChannelNumber

    allocate(estObsERR(tvs_nsensors, tvs_maxChannelNumber, obs_numheader(obsSpaceData)))
    allocate(obsErrSum(tvs_nsensors, tvs_maxChannelNumber))
    allocate(dataCountPerChan(tvs_nsensors, tvs_maxChannelNumber))
    allocate(meanObsErr(tvs_nsensors, tvs_maxChannelNumber))
    allocate(residualSqrd(tvs_nsensors, tvs_maxChannelNumber))
      ...
      if (tvs_isIdBurpTovs(idatyp)) then
        ...
        do bodyIndex = idata, idatend
          if (obs_bodyElem_i( obsspacedata, OBS_ASS, bodyIndex ) == obs_assimilated) then
            ...
            dataCountPerChan(sensorIdIndex, channelIndex) = dataCountPerChan(sensorIdIndex, channelIndex) + 1

            ! observation error                                   
            estObsERR(sensorIdIndex, channelIndex, dataCountPerChan(sensorIdIndex, channelIndex)) = &
                                                 obs_bodyElem_r(obsspacedata, OBS_VAR, bodyIndex) - &
                                                 obs_bodyElem_r(obsspacedata, OBS_TRUO, bodyIndex)

            ! Sum of the observation error (per sensor, per channel index)
            obsErrSum(sensorIdIndex, channelIndex) = obsErrSum(sensorIdIndex, channelIndex) + &
                  estObsERR(sensorIdIndex, channelIndex, dataCountPerChan(sensorIdIndex, channelIndex))
          end if
        end do
      end if
    ...
    ! Compute mean observation error (per sensor, per channel index)
    do sensoridIndex = 1, tvs_nsensors
      loopChannelIndex1: do channelIndex = 1, tvs_maxChannelNumber
        if (dataCountPerChan(sensorIdIndex, channelIndex) == 0) cycle loopChannelIndex1
        meanObsErr(sensorIdIndex, channelIndex) = obsErrSum(sensorIdIndex, channelIndex) / &
                                                  dataCountPerChan(sensorIdIndex, channelIndex)
      end do loopChannelIndex1
    end do

    ! squared of residual= sum(mean observation error - mean observation error)
    do sensoridIndex = 1, tvs_nsensors
      loopChannelIndex2: do channelIndex = 1, tvs_maxChannelNumber
        if (dataCountPerChan(sensorIdIndex, channelIndex) == 0) cycle loopChannelIndex2
        
        do obsIndex = 1, dataCountPerChan(sensorIdIndex, channelIndex)
          residualSqrd(sensorIdIndex, channelIndex) = residualSqrd(sensorIdIndex, channelIndex) + &
                                          (estObsERR(sensorIdIndex, channelIndex, obsIndex) - &
                                          meanObsErr(sensorIdIndex, channelIndex))**2
        end do
      end do loopChannelIndex2
    end do

    ! Compute observation error Stdev. (per sensor, per channel index)
    do sensoridIndex = 1, tvs_nsensors
      loopChannelIndex3: do channelIndex = 1, tvs_maxChannelNumber
        if (dataCountPerChan(sensorIdIndex, channelIndex) == 0) cycle loopChannelIndex3
        obsErrStdev(sensorIdIndex, channelIndex) = SQRT(residualSqrd(sensorIdIndex, channelIndex) / &
                                                  dataCountPerChan(sensorIdIndex, channelIndex))
        write(*,*) 'ZQ_obsErrStdev', obsErrStdev(sensorIdIndex, channelIndex), sensorIdIndex, channelIndex
      end do loopChannelIndex3
    end do



    deallocate(estObsERR)
