...
module BCovarSetupChem_mod 
  ...
  ! bcsc_readbgStats
  !--------------------------------------------------------------------------
  subroutine bcsc_readbgStats(kmode)
    !
    ! :Purpose: Read content of 'bCovarSetupChem_out.fst' and place in
    !           bgStats structure. Provided for user convenience - not used 
    !           by main MIDAS programs.
    !
    ...
    !Locals
    integer :: ierr,varIndex,length,nulbgst,levelIndex,i
    integer :: fnom, fclos, fstouv, fstfrm
    real(8) :: readarray(3000)

    ! standard fst file variables
    integer :: ini,inj,ink,startPosition
    integer :: ip1,ip2,ip3
    integer :: idateo
    ...
    real(8), pointer :: vlev_T(:)
    ! Reference surface pressure
    real(8), parameter :: zps = 101000.D0
    integer :: nlev_T,maxnlev

    character(len=100), parameter :: fname='bCovarSetupChem_out.fst'
    logical :: fileExists
    
    logical :: ReadAbort = .false.
    
    call bcsc_finalize()
    bgStats%initialized = .false.
    
    ! Impose waiting on reference CPU 0 so that file is available (if to be produced).
    ! Needed when there is a possibility that some CPU will reach this stage
    ! before the stats file has been generated and completed by CPU 0. 
    ! This occurs with obsSelection.Abs (kmode=1).
    call rpn_comm_bcast(bgStats%initialized,1,'MPI_LOGICAL',0,'GRID',ierr)

    inquire(file=fname,exist=fileExists)
             
    nulbgst = 0
    if ( fileExists ) then
      ierr = fnom(nulbgst,fname,'STD+OLD+R/O',0)
      if ( ierr == 0 ) then
        ierr =  fstouv(nulbgst,'RND+OLD')
      else
        call utl_abort('bcsc_readbgStats: No background stats file for CH. ' // &
                       'Check if WritePhysSpaceStats=.true. ' // &
		       '(and getPhysSpaceHCorrel=.true.) for nml NAMBCHM')
      end if
    else
      call utl_abort('bcsc_readbgStats: No background stats file for CH. ' // &
                     'Check if WritePhysSpaceStats=.true. ' // &
    ...
    ! Read vertical levels (pressure in Pa)

    ierr = utl_fstlir(readarray,nulbgst,INI,INJ,INK,-1,'Pressure',-1,-1,-1, &
                      ' ','PX')
    if ( ierr < 0 ) then
      call utl_abort('bcsc_readbgStats: File writing not completed or ' // &
                     'write error.')
    end if
    allocate(bgStats%vlev(INK))
    bgStats%vlev(:)=readarray(1:INK)
    bgStats%nlev = INK
    
    ! Read lat and long of input error std dev field (in radians)
    
    ierr = utl_fstlir(readarray,nulbgst,INI,INJ,INK,-1,'latitude',-1,-1,-1, &
                      ' ','^^')
    if ( ierr < 0 ) then
      call utl_abort('bcsc_readbgStats: File writing not completed or ' // &
                     'write error.')
    end if
    allocate(bgStats%lat(INJ))
    bgStats%lat(:)=readarray(1:INJ)
    bgStats%nj = INJ
    
    ierr = utl_fstlir(readarray,nulbgst,INI,INJ,INK,-1,'longitude',-1,-1,-1, &
                      ' ','>>')
    if ( ierr < 0 ) then
      call utl_abort('bcsc_readbgStats: File writing not completed or write error.')
    end if  
    allocate(bgStats%lon(INI))
    bgStats%lon(:)=readarray(1:INI)
    
    ! Assumes input longitude array contains an extra longitude 
    ! to cover 0 and 360 (in radians)
    bgStats%ni = INI-1  
        
    ! Set reference pressure levels of trial for a surface pressure of zps
    call vco_SetupFromFile( vco_trl, './trlm_01' )
    ierr = vgd_levels( vco_trl%vgrid, ip1_list=vco_trl%ip1_T, levels=vlev_T, &
                       sfc_field=zps, in_log=.false.)
    nlev_T = vco_trl%nlev_T
    
    ! Set remaining allocations
    
    maxnlev=max(nlev_T,bgStats%nlev)
    allocate(bgStats%stddev(bgStats%ni+1,bgStats%nj, &
             bgStats%nkgdim ))
    allocate(bgStats%hcorrlen(maxnlev,bgStats%numvar3d+bgStats%numvar2d ))
    allocate(bgStats%corvert(maxnlev, maxnlev, bgStats%numvar3d))
    allocate(bgStats%corverti(maxnlev, maxnlev, bgStats%numvar3d))
    allocate(bgStats%invsum(maxnlev, bgStats%numvar3d))

    ! Read physical space correlation matrices and their inverses
    ! as well as "1 / sums of vertical correlations rows"
    ...
    do varIndex = 1, bgStats%numvar3d
      ...
      length = bgStats%nlev
      nomvar = bgStats%varNameList(varIndex)

      ! Read vertical correlation matrix
  
      ierr = utl_fstlir(bgStats%corvert(1:length,1:length,varIndex), &
             nulbgst,INI,INJ,INK,idateo,'CORVERT',ip1,ip2,ip3,typvar,nomvar)
      if (ierr < 0 ) then
        write(*,*) 'bcsc_readbgStats: CORVERT matrix not found in stats ', &
	           'file for variable ', nomvar
        ReadAbort = .true.
      end if        
      ...
      do i = 1, length 
        bgStats%invsum(i,varIndex) = 1.0d0/sum(bgStats%corvert(1:length,i,varIndex)) 
      end do
   
    end do
    
    ! Reset bgStats%corvert and bgStats%invsum if vertical 
    ! dimensions differ from that of trial fields
        
    if ( bgStats%nlev /= nlev_T ) call bcsc_resetCorvert(nlev_T,vlev_T)

    ! Read error std dev and horizontal corelation length
    ...
    do varIndex = 1,bgStats%numvar3d+bgStats%numvar2d
      ...
      ierr = utl_fstlir(bgStats%hcorrlen(1:length,varIndex),nulbgst, &
                        INI,INJ,INK,idateo,'HCORRLEN',ip1,ip2,ip3,'X', &
			bgStats%varNameList(varIndex))
      if (ierr < 0 ) then
        write(*,*) 'bcsc_readbgStats: HCORRLEN not found in stats file ', &
	           'for variable ', bgStats%varNameList(varIndex)
        bgStats%hcorrlen(1:length,varIndex)=200.00
        ! ReadAbort = .true.
      end if        

      do levelIndex=1,length
        ierr = utl_fstlir(bgStats%stddev(1:bgStats%ni+1,1:bgStats%nj, &
	                  startPosition-1+levelIndex),nulbgst, &
                          INI,INJ,INK,idateo,'STDDEV',levelIndex, &
			  ip2,ip3,'X',bgStats%varNameList(varIndex))
        if (ierr < 0 ) then
          write(*,*) 'bcsc_readbgStats: STDDEV not found in stats file ', &
	             'for variable ', bgStats%varNameList(varIndex)
          ReadAbort = .true.
        end if        
      end do
    ...
    ! Reset bgStats%hcorrlen if vertical dimensions differ from that of trial fields
    ! Note: bgkStats%stddev would be interpolated elsewhere
        
    if ( bgStats%nlev /= nlev_T ) call bcsc_resetHcorrlen(nlev_T,vlev_T)
    ...
    if (ReadAbort) then
      call utl_abort('bcsc_readBgchStats: Error(s) ' // &
                     'in reading backgound stats')
    end if 
    
  end subroutine bcsc_readbgStats
