...
contains
  ...
  !---------------------------------------------------------
  ! calcGeopotHeight_gsv_nl_vcode2100x_r4
  !---------------------------------------------------------
  subroutine calcGeopotHeight_gsv_nl_vcode2100x_r4(statevector, Z_T, Z_M)
    ...
    real(4), allocatable   :: Hsfc_r4(:,:), HsfcLS_r4(:,:)
    real(4), pointer       :: GZHeightM_out(:,:,:), GZHeightT_out(:,:,:)
  ...
  ! gz2alt_gsv_r4
  !---------------------------------------------------------
  subroutine gz2alt_gsv_r4(statevector, Z_MT, skipDiagLevel)
    !
    ...
    do stepIndex = 1, statevector%numStep
      do latIndex = statevector%myLatBeg, statevector%myLatEnd
        do lonIndex = statevector%myLonBeg, statevector%myLonEnd
          latitude = hco%lat2d_4(lonIndex,latIndex)
          gzH(:) = Z_MT(lonIndex, latIndex, 1:levEnd, stepIndex)
          Z_MT(lonIndex, latIndex, 1:levEnd, stepIndex) = phf_convertGZtoZ(gzH,latitude,levEnd)
        end do
      end do
    end do
  ...
  !---------------------------------------------------------
  ! calcGeopotHeight_gsv_nl_vcode2100x_r8
  !---------------------------------------------------------
  subroutine calcGeopotHeight_gsv_nl_vcode2100x_r8(statevector, Z_T, Z_M)
    ...
    do stepIndex = 1, numStep
      if (gsv_getVco(statevector)%sleveCoord) then
        HsfcLS => gsv_getHeightSfcLS(statevector)
        call fetch3DLevels_r8(gsv_getVco(statevector), sfcFld=Hsfc, sfcFldLS_opt=HsfcLS, &
                              fldM_opt=GZHeightM_out, fldT_opt=GZHeightT_out)
      else
  ...
  ! gz2alt_gsv_r8
  !---------------------------------------------------------
  subroutine gz2alt_gsv_r8(statevector, Z_MT, skipDiagLevel)
    !
    ! :Purpose: Conversion of geopotential height to geometric
    !           altitude.
    !           real(8) version
    ...
    do stepIndex = 1, statevector%numStep
      do latIndex = statevector%myLatBeg, statevector%myLatEnd
        do lonIndex = statevector%myLonBeg, statevector%myLonEnd
          latitude = real(hco%lat2d_4(lonIndex,latIndex),8)
          gzH(:) = Z_MT(lonIndex, latIndex, 1:levEnd, stepIndex)
          Z_MT(lonIndex, latIndex, 1:levEnd, stepIndex) = phf_convertGZtoZ(gzH,latitude,levEnd)
        end do
      end do
  ...
  subroutine calcHeight_gsv_nl_vcode5xxx( statevector, &
    ...
    do_computeHeight_gsv_nl : do stepIndex = 1, numStep
      !$OMP PARALLEL DO PRIVATE(latIndex, lonIndex, height_T, height_M, lat_4, lat_8, sLat, lev_T, &
  ...
  subroutine calcHeight_gsv_tl(statevector,statevectorRef)
    ...
    contains
      ...
      subroutine calcHeight_gsv_tl_vcode2100x
        ...
        call msg('calcHeight_gsv_tl_vcode2100x (czp)', 'END', verb_opt=4)
      ...
      subroutine calcHeight_gsv_tl_vcode5xxx
        ...
        Vcode_anl = vco_getVcode(gsv_getVco(statevectorRef))
  ...
  subroutine calcPressure_gsv_nl_vcode2100x(statevector, &
    ...
    !
    !- Compute geopotential heights and set pointers
    !
    if ( statevector%dataKind == 4 ) then
      allocate(height_T_ptr_r4(statevector%myLonBeg:statevector%myLonEnd, &
                               statevector%myLatBeg:statevector%myLatEnd, &
                               nlev_T, statevector%numStep))
      allocate(height_M_ptr_r4(statevector%myLonBeg:statevector%myLonEnd, &
                               statevector%myLatBeg:statevector%myLatEnd, &
                               nlev_M, statevector%numStep))
      ...
      allocate(height_T_ptr_r8(statevector%myLonBeg:statevector%myLonEnd, &
                               statevector%myLatBeg:statevector%myLatEnd, &
                               nlev_T, statevector%numStep))
      allocate(height_M_ptr_r8(statevector%myLonBeg:statevector%myLonEnd, &
                               statevector%myLatBeg:statevector%myLatEnd, &
                               nlev_M, statevector%numStep))
    ...
    if ( statevector%dataKind == 4 ) then
      deallocate(height_T_ptr_r4)
      deallocate(height_M_ptr_r4)
    else
      deallocate(height_T_ptr_r8)
      deallocate(height_M_ptr_r8)
    end if
  ...
  subroutine calcPressure_gsv_nl_vcode5xxx_r8(statevector, P_T, P_M)
    ...
    real(8), pointer       :: PressureM_out(:,:,:), PressureT_out(:,:,:)
    real(8), pointer       :: field_Psfc(:,:,:,:), field_PsfcLS(:,:,:,:)
    ...
    do stepIndex = 1, numStep
      ...
      if (Vcode == 5100) then
        ...
        call fetch3DLevels_r8(gsv_getVco(statevector), Psfc, sfcFldLS_opt=PsfcLS, &
                              fldM_opt=PressureM_out, fldT_opt=PressureT_out)
      else
        call fetch3DLevels_r8(gsv_getVco(statevector), Psfc, &
                              fldM_opt=PressureM_out, fldT_opt=PressureT_out)
      end if
  ...
  subroutine calcPressure_gsv_nl_vcode5xxx_r4(statevector, P_T, P_M)
    ...
    ! Locals:
    real(4), allocatable   :: Psfc(:,:), PsfcLS(:,:)
    real(4), pointer       :: PressureM_out(:,:,:), PressureT_out(:,:,:)
    real(4), pointer       :: field_Psfc(:,:,:,:), field_PsfcLS(:,:,:,:)
    integer                :: stepIndex, numStep, Vcode
  ...
  subroutine calcPressure_gsv_tl(statevector, statevectorRef)
    ...
    contains
      ...
      subroutine calcPressure_gsv_tl_vcode2100x
        ...
        real(8), pointer     :: P_M_ptr(:,:,:,:)
        real(8), pointer     :: Z_T_ptr(:,:,:,:)
        ...
        allocate(Z_T_ptr(statevectorRef%myLonBeg:statevectorRef%myLonEnd, &
                         statevectorRef%myLatBeg:statevectorRef%myLatEnd, &
                         nlev_T, numStep))
        allocate(Z_M_ptr(statevectorRef%myLonBeg:statevectorRef%myLonEnd, &
                         statevectorRef%myLatBeg:statevectorRef%myLatEnd, &
                         nlev_M, numStep))
        ...
        nullify(delP_M_ptr_r48)
        nullify(delPsfc_ptr_r48)
        nullify(tt_ptr)
        nullify(hu_ptr)
        nullify(P_T_ptr)
        nullify(P_M_ptr)
        ...
        call gsv_getField(statevector,delP_M_ptr_r48,'P_M')
        call gsv_getField(statevector,delPsfc_ptr_r48,'P0')
        call gsv_getField(statevectorRef,tt_ptr,'TT')
        call gsv_getField(statevectorRef,hu_ptr,'HU')
        call gsv_getField(statevectorRef,P_T_ptr,'P_T')
        call gsv_getField(statevectorRef,P_M_ptr,'P_M')
        ...
        do_computePressure_gsv_tl: do stepIndex = 1, numStep
          do latIndex = statevector%myLatBeg, statevector%myLatEnd
            do lonIndex = statevector%myLonBeg, statevector%myLonEnd
              ...
              p_T   = p_T_ptr(lonIndex,latIndex,nlev_T,stepIndex)
              p_M   = p_M_ptr(lonIndex,latIndex,nlev_M,stepIndex)
              ...
              delP_T(nlev_T) = delP0*delP_delP0 + delTV*delP_delTV

              ! Momentum diagnostic level, NL: pressure = P0*exp(-Rgh*dh/MPC_RGAS_DRY_AIR_R8/tv)
              dh = Z_M - rMT
              Rgh = ec_rg
              delP_delP0 = p_M/P0
              delP_delTV = p_M*Rgh*dh/MPC_RGAS_DRY_AIR_R8/tv**2
              delP_M(nlev_M) = delP0*delP_delP0 + delTV*delP_delTV
        ...
        do_computePressure_gsv_ad: do stepIndex = 1, numStep
          do latIndex = statevector%myLatBeg, statevector%myLatEnd
            do lonIndex = statevector%myLonBeg, statevector%myLonEnd
              ...
              delP_M(1:nlev_M)=delP_M_ptr_r48(lonIndex,latIndex,1:nlev_M,stepIndex)
  ...
  !---------------------------------------------------------
  ! calcGeopotHeight_col_nl_vcode2100x
  !---------------------------------------------------------
  subroutine calcGeopotHeight_col_nl_vcode2100x(column, Z_T, Z_M)
    !
    ! :Purpose: Return geopotential heights on a GEM-H column.
    !
    implicit none
    ...
    deallocate(hSfc)
    call msg('calcGeopotHeight_col_nl_vcode2100x (czp)', 'END', verb_opt=4)
  end subroutine calcGeopotHeight_col_nl_vcode2100x

  !---------------------------------------------------------
  ! gz2alt_col
  !---------------------------------------------------------
  subroutine gz2alt_col(column, Z_MT, skipDiagLevel)
    !
    ! :Purpose: Conversion of geopotential height to geometric
    !           altitude.
    !
    ...
    do colIndex = 1, numCol
      latitude = real(col_getLat(column, colIndex),8)
      gzH(:) = Z_MT(1:levEnd, colIndex)
      Z_MT(1:levEnd, colIndex) = phf_convertGZtoZ(gzH,latitude,levEnd)
    end do
  ...
  subroutine calcHeight_col_tl(columnInc,columnIncRef)
    ...
    contains
      ...
      subroutine calcHeight_col_tl_vcode2100x
        ...
        call msg('calcHeight_col_tl_vcode2100x (czp)', 'START', verb_opt=4)
  ...
  subroutine calcPressure_col_nl_vcode2100x(column, P_T, P_M)
    ...
    call calcGeopotHeight_col_nl_vcode2100x(column, Z_T_ptr, Z_M_ptr)
    ...
    do_onAllcolumns: do colIndex = 1, col_getNumCol(column)
      ...
      do lev_M = nlev_M-1, 1, -1
        ...
        Z_M1 = Z_M_ptr(lev_M+1,colIndex)
        Z_T  = Z_T_ptr(lev_T  ,colIndex)
  ...
  subroutine calcPressure_col_nl_vcode5xxx(column, P_T, P_M)
    ...
    ! Locals:
    real(8), allocatable :: Psfc(:,:), PsfcLS(:,:)
    real(8), pointer     :: zppobsM(:,:,:), zppobsT(:,:,:)
    ...
    if (Vcode == 5100) then
      ...
      call fetch3DLevels_r8(col_getVco(column), Psfc ,sfcFldLS_opt=PsfcLS, &
                            fldM_opt=zppobsM, fldT_opt=zppobsT)
      deallocate(PsfcLS)
    else
      call fetch3DLevels_r8(col_getVco(column), Psfc ,fldM_opt=zppobsM, fldT_opt=zppobsT)
    end if
    ...
    contains
      ...
      subroutine calcPressure_col_tl_vcode2100x
        ...
        do_onAllcolumns: do colIndex = 1, numCol
          ...
          !- Compute pressure on diagnostic levels
          !
          rMT = col_getHeight(columnIncRef, 1, colIndex, 'SF') ! surface height

          hu  = col_getElem(columnIncRef, nLev_T, colIndex, 'HU')
          tt  = col_getElem(columnIncRef, nLev_T, colIndex, 'TT')
          p_T = col_getElem(columnIncRef, nLev_T, colIndex, 'P_T')
          p_M = col_getElem(columnIncRef, nLev_M, colIndex, 'P_M')
          ...
          delTT = col_getElem(columnInc, nLev_T, colIndex, 'TT')
          delHU = col_getElem(columnInc, nLev_T, colIndex, 'HU')
          delP0 = col_getElem(columnInc,      1, colIndex, 'P0')
          ...
          !- Fill the pressure increment array
          !
          delP_T_ptr(1:nlev_T,colIndex)=delP_T(1:nlev_T)
          delP_M_ptr(1:nlev_M,colIndex)=delP_M(1:nlev_M)
    ...
    contains
      ...
      subroutine calcPressure_col_ad_vcode2100x
        ...
        do_onAllcolumns: do colIndex = 1, numCol
          ...
          !- Fill the pressure increment array
          !
          delP_T(1:nlev_T) = delP_T_ptr(1:nlev_T,colIndex)
          delP_M(1:nlev_M) = delP_M_ptr(1:nlev_M,colIndex)
  ...
  subroutine czp_ensureCompatibleTops(vco_sourceGrid,vco_destGrid)
    ...
    if ( vco_getVcode(vco_sourceGrid) == 21001 ) then
      ...
      if (vco_getVcode(vco_sourceGrid) == 5100) then
      ...
      if (vco_getVcode(vco_destGrid) == 5100) then
      ...
      do colIndex = 1, numColumns
        do lev_T = 1, (nlev_T-1)
          if ( lev_T == 1 ) then
            ...
            ! Gravity acceleration
            lat_8 = col_getLat(column, colIndex)
            sLat = sin(lat_8)
            Rgh = phf_gravityalt(sLat, height_T)
