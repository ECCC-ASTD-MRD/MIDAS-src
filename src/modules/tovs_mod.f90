...
contains
    ...
    if (radiativeTransferCode == 'RTTOV') then
      ...
      do sensorIndex=1, tvs_nsensors
        ...
        if (runObsOperatorWithHydrometeors) then
          ...
          if (errorStatus /= errorStatus_success) then
            write(*,*) 'rttov_read_scattcoeffs: fatal error reading RTTOV-SCATT coefficients for sensor ', sensorIndex
            call utl_abort('tvs_setupAlloc')
          end if
        end if
    ...
    sensor_loop:do sensorIndex = 1, tvs_nsensors
      ...
      if (btCountScatt > 0) then
        ...
        call tvs_rttov_scatt_setupindex(                   &
            ...
            lchannelSubset )                                  ! OPTIONAL input array of logical flags to indicate a subset of channels
    ...
    do profileIndex = 1, nprofiles
      ...
      do channelIndex1 = 1, n_chan
        ...
        if (polarisedScattering) then
          ...
          !   2) Polarisation ID eq 4,5,6 or 7 (single V or H polarisation only)
          !   3) Polarisation ID different from last channel
  ...
  subroutine tvs_printDetailledOmfStatistics(obsSpaceData)
    !
    ! :Purpose: Print channel by channel O-F statistics for radiances
    !
    implicit none
