...
module gridStateVector_mod
  ...
  ! arrays used for transpose VarsLevs <-> LatLon
      ...
      if (statevector%UVComponentPresent) then
        ...
        do kIndex = k1UV, k2UV
          do stepIndex = 1, statevector%numStep
            do latIndex = lat1, lat2
              do lonIndex = lon1, lon2
                statevector%gdUV_r8(lonIndex,latIndex,kIndex,stepIndex) = 0.0d0
              end do
            end do
          end do
        end do
      ...
      if (statevector%UVComponentPresent) then
        ...
        do kIndex = k1UV, k2UV
          do stepIndex = 1, statevector%numStep
            do latIndex = lat1, lat2
              do lonIndex = lon1, lon2
                statevector%gdUV_r4(lonIndex,latIndex,kIndex,stepIndex) = 0.0d0
              end do
            end do
          end do
        end do
  ...
  subroutine gsv_interpolate(statevector_in,statevector_out, &
    ...
    call gsv_transposeLatLonToVarsLevs( statevector_in, statevector_in_VarsLevs )
    ...
    call gsv_transposeVarsLevsToLatLon( statevector_in_varsLevs_hInterp, statevector_in_hInterp )
  ...
  function gsv_getFieldUV_r8(statevector) result(field)
    ...
    lon1 = statevector%myLonBeg
    lat1 = statevector%myLatBeg
    k1 = statevector%myUVkBeg

    if (.not. associated(statevector%gdUV_r8)) call utl_abort('gsv_getFieldUV_r8: data with type r8 not allocated')
  ...
  function gsv_getFieldUV_r4(statevector) result(field)
    ...
    if (.not. associated(statevector%gdUV_r4)) call utl_abort('gsv_getFieldUV_r4: data with type r4 not allocated')
  ...
  subroutine gsv_readFromFileAndTransposeToTiles(statevector_out, fileName,  &
    ...
    ! locals
    type(struct_gsv) :: statevector_file, statevector_tiles
    ...
    ! initialize single precision 3D working copy of statevector for reading file
    call gsv_allocate(statevector_file, 1, statevector_out%hco, statevector_out%vco, &
                      dateStamp_opt=statevector_out%datestamplist(stepIndex), &
                      mpi_local_opt=.true., mpi_distribution_opt='VarsLevs',  &
                      dataKind_opt=4, allocGZsfc_opt=readGZsfc,               &
                      varNames_opt=varNamesToRead)
    ...
    !-- 4.0 Unit conversion
    if ( unitConversion ) then
      call gsv_fileUnitsToStateUnits( statevector_file, containsFullField )
    end if
    ...
    call gsv_transposeVarsLevsToLatLon(statevector_file, statevector_tiles)

    !-- 6.0 Copy result to output statevector

    if ( associated(statevector_tiles%gzSfc) .and. associated(statevector_out%gzSfc) ) then
      statevector_out%gzSfc(:,:) = statevector_tiles%gzSfc(:,:)
    end if

    do varIndex = 1, vnl_numvarmax
      if ( .not. gsv_varExist(statevector_out,vnl_varNameList(varIndex)) ) cycle
      field_in_ptr => gsv_getField_r8(statevector_tiles, vnl_varNameList(varIndex))
      field_out_ptr => gsv_getField_r8(statevector_out, vnl_varNameList(varIndex))
  ...
  !--------------------------------------------------------------------------
  subroutine gsv_readFromFileAndInterp1Proc(statevector_out, fileName,  &
             vco_file, hco_file, etiket_in, typvar_in, stepIndex, unitConversion,  &
             readGZsfc, containsFullField)
    implicit none
    ! Note this routine currently only works correctly for reading FULL FIELDS,
    ...
    ! locals
    type(struct_gsv) :: statevector_file, statevector_hinterp, statevector_vinterp
    ...
    call gsv_vInterpolate_r4(statevector_hinterp,statevector_vinterp)

    call gsv_deallocate(statevector_hinterp)

    !-- 5.0 Copy result to output statevector and convert HU to LQ
    ...
    do varIndex = 1, vnl_numvarmax
      if ( .not. gsv_varExist(statevector_out,vnl_varNameList(varIndex)) ) cycle
      field_in_ptr => gsv_getField_r4(statevector_vinterp, vnl_varNameList(varIndex))
      field_out_ptr => gsv_getField_r4(statevector_out, vnl_varNameList(varIndex))
  ...
  subroutine gsv_transposeVarsLevsToLatLon(statevector_in, statevector_out)
    ...
    if ( sendrecvKind == 4 ) then
      call utl_reAllocate( gd_send_varsLevs_r4, statevector_out%lonPerPEmax, statevector_out%latPerPEmax, &
                           maxkCount, statevector_out%numStep, mpi_nprocs )
  ...
  subroutine gsv_transposeLatLonToVarsLevs(statevector_in, statevector_out)
    ...
    call tmg_start(153,'GSV_TRANSVARSLEVS1')
    ...
    if ( sendrecvKind == 4 ) then
      call utl_reAllocate( gd_send_varsLevs_r4, statevector_in%lonPerPEmax, statevector_in%latPerPEmax, &
                           maxkCount, statevector_in%numStep, mpi_nprocs )
    ...
    call tmg_start(154,'GSV_TRANSVARSLEVS2')
    ...
    call tmg_start(155,'GSV_TRANSVARSLEVS3')
    ...
    call tmg_start(156,'GSV_TRANSVARSLEVS4')
    ...
    if ( gsv_varExist(stateVector_out, 'UU') .and.  &
      ...
      if ( outKind /= 8 ) then
        call utl_abort('gsv_transposeLatLonToVarsLevs: only compatible with real*8 output')
      end if
      ...
      LOOP_KINDEX: do kIndexUU = 1, stateVector_out%nk
        ...
        nsize = statevector_out%ni * statevector_out%nj * statevector_out%numStep
        mpiTagUU = kIndexUU
        mpiTagVV = kIndexVV
      ...
      if ( statevector_out%UVComponentPresent ) then
        do kIndex = statevector_out%myUVkBeg, statevector_out%myUVkEnd
          do stepIndex = 1, statevector_out%numStep
            statevector_out%gdUV_r8(:, :, kIndex, stepIndex) =  gdUV_r8(:, :, stepIndex, kIndex)
          end do
        end do
        deallocate(gdUV_r8)
        deallocate(gd_r8)
  ...
  !--------------------------------------------------------------------------
  ! gsv_transposeLatLonToVarsLevsAd
  !--------------------------------------------------------------------------
  subroutine gsv_transposeLatLonToVarsLevsAd(statevector_in, statevector_out)
    ...
    if ( statevector_in%mpi_distribution /= 'VarsLevs' ) then
      call utl_abort('gsv_transposeLatLonToVarsLevsAd: input statevector must have VarsLevs mpi distribution') 
    end if

    if ( statevector_out%mpi_distribution /= 'Tiles' ) then
      call utl_abort('gsv_transposeLatLonToVarsLevsAd: out statevector must have Tiles mpi distribution')
    end if
    ...
    call tmg_start(157,'GSV_TRANSVARSLEVSAD1')
    ...
    if ( gsv_varExist(stateVector_in, 'UU') .and.  &
      ...
      if ( outKind /= 8 ) then
        call utl_abort('gsv_transposeLatLonToVarsLevsAd: only compatible with real*8 output')
      end if

      if ( statevector_in%UVComponentPresent ) then
        allocate(gdUV_r8(statevector_in%ni,statevector_in%nj,statevector_in%numStep,  &
                         statevector_in%myUVkBeg:statevector_in%myUVkEnd))
        allocate(gd_r8(statevector_in%ni,statevector_in%nj,statevector_in%numStep,  &
                       statevector_in%myUVkBeg:statevector_in%myUVkEnd))
      end if
      ...
      LOOP_KINDEX: do kIndexUU = 1, stateVector_in%nk
        ...
        if ( MpiIdUU == MpiIdVV .and.  mpi_myid == MpiIdUU ) then
          do stepIndex = 1, statevector_in%numStep
            gd_r8(:, :, stepIndex, kIndexUU) = statevector_in%gdUV_r8(:, :, kIndexVV, stepIndex)
            gd_r8(:, :, stepIndex, kIndexVV) = statevector_in%gdUV_r8(:, :, kIndexUU, stepIndex)
          end do
          cycle LOOP_KINDEX
        end if

        ! need to do mpi communication to exchange wind components
        nsize = statevector_in%ni * statevector_in%nj * statevector_in%numStep
        mpiTagUU = kIndexUU
        mpiTagVV = kIndexVV
        ...
        if ( mpi_myid == MpiIdUU ) then ! I have UU
          ...
          numSend = numSend + 1
          do stepIndex = 1, statevector_in%numStep
            gdUV_r8(:, :, stepIndex, kIndexUU) = statevector_in%gdUV_r8(:, :, kIndexUU, stepIndex)
          end do
          ...
          numSend = numSend + 1
          do stepIndex = 1, statevector_in%numStep
            gdUV_r8(:, :, stepIndex, kIndexVV) = statevector_in%gdUV_r8(:, :, kIndexVV, stepIndex)
          end do
    ...
    call tmg_start(158,'GSV_TRANSVARSLEVSAD2')
    ...
    call tmg_start(159,'GSV_TRANSVARSLEVSAD3')
    nsize = statevector_out%lonPerPEmax * statevector_out%latPerPEmax * statevector_out%numStep * maxkCount
    if (mpi_nprocs > 1) then
      if ( sendrecvKind == 4 ) then
        call rpn_comm_alltoall(gd_send_varsLevs_r4, nsize, 'mpi_real4',  &
    ...
    call tmg_start(160,'GSV_TRANSVARSLEVSAD4')
    if ( sendrecvKind == 4 .and. outKind == 4 ) then
      field_out_r4_ptr => gsv_getField_r4(statevector_out)
      !$OMP PARALLEL DO PRIVATE(yourid)
      do yourid = 0, (mpi_nprocs-1)
  ...
  end subroutine gsv_transposeLatLonToVarsLevsAd
      ...
      do stepIndex = 1, statevector_out%numStep
        K_LOOP: do kIndex = statevector_in%mykBeg, statevector_in%mykEnd
          ...
          if ( trim(varName) == 'UU' ) then
            ...
            fieldVV_out_r8_ptr => gsv_getFieldUV_r8(statevector_out)
            ierr = utl_ezuvint( fieldUU_out_r8_ptr(:,:,kIndex,stepIndex), fieldVV_out_r8_ptr(:,:,kIndex,stepIndex), &
            ...
            fieldUU_out_r8_ptr => gsv_getFieldUV_r8(statevector_out)
      ...
      do stepIndex = 1, statevector_out%numStep
        K_LOOP: do kIndex = statevector_in%mykBeg, statevector_in%mykEnd
          ...
          if ( trim(varName) == 'UU' ) then
            ...
            fieldVV_out_r4_ptr => gsv_getFieldUV_r4(statevector_out)
            ierr = utl_ezuvint( fieldUU_out_r4_ptr(:,:,kIndex,stepIndex), fieldVV_out_r4_ptr(:,:,kIndex,stepIndex),   &
            ...
            fieldUU_out_r4_ptr => gsv_getFieldUV_r4(statevector_out)
  ...
  ! gsv_readTrials
  !--------------------------------------------------------------------------
  subroutine gsv_readTrials(stateVector)
    implicit none
    ...
    ! abort if not enough mpi tasks
    if ( mpi_nprocs < stateVector%numStep ) then
      write(*,*) 'gsv_readTrials: number of trial time steps, mpi tasks = ', stateVector%numStep, mpi_nprocs
      call utl_abort('gsv_readTrials: number of mpi tasks must be at least as large as number of trial time steps')
    end if

    ! figure out which time step I will read, if any (-1 if none)
    stepIndexToRead = -1
    do stepIndex = 1, stateVector%numStep
      procToRead = nint( real(stepIndex-1) * real(mpi_nprocs) / real(stateVector%numStep) )
    ...
    nullify(varNamesToRead)
    call vnl_varNamesFromExistList(varNamesToRead, stateVector%varExistlist(:))

    call tmg_start(180,'READTRIALS_1')
    ! loop over all times for which stateVector is allocated
    if ( stepIndexToRead /= -1 ) then
      dateStamp = stateVector%dateStampList(stepIndexToRead)
      ...
      call gsv_allocate( stateVector_1step, 1, stateVector%hco, stateVector%vco,  &
                         dateStamp_opt=stateVector%dateStampList(stepIndexToRead),  &
                         mpi_local_opt=.false., dataKind_opt=4,  &
                         allocGZsfc_opt=.true., varNames_opt=varNamesToRead, &
                         hInterpolateDegree_opt=stateVector%hInterpolateDegree )

      ! read the trial file for this timestep
      call tmg_start(189,'READTRIALS_RAM')
      fileName = ram_fullWorkingPath(fileName)
      call tmg_stop(189)
      call gsv_readFromFile(stateVector_1step, fileName, ' ', 'P',  &
                            readGZsfc_opt=.true.)
    ...
    if ( stateVector%mpi_distribution == 'VarsLevs' ) then
      call gsv_transposeStepToVarsLevs(stateVector_1step, stateVector)
    else if ( stateVector%mpi_distribution == 'Tiles' ) then
      call gsv_transposeStepToTiles(stateVector_1step, stateVector)
    else
      call utl_abort( 'gsv_readTrials: not compatible with mpi_distribution = ' // &
                      trim(stateVector%mpi_distribution) )
    end if
  ...
  !--------------------------------------------------------------------------
  subroutine gsv_transposeStepToVarsLevs(stateVector_1step, stateVector_VarsLevs)
    implicit none
    ! arguments
