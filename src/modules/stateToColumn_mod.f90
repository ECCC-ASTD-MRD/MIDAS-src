...
contains
  ...
  subroutine rejectZeroWeightObs(intInfo, obsSpaceData)
    ...
    ! Locals:
    integer :: myHeaderIndex, headerIndex, varLevIndex, procIndex
    integer :: numVarLev, myNumHeader, myNumHeaderMax, bodyIndexBeg, bodyIndexEnd, bodyIndex
    integer :: depotIndex, ierr, nsize
    ...
    call rpn_comm_allreduce(maxval(myNumHeaders(:)), myNumHeaderMax, 1,  &
    ...
    nsize = myNumHeaderMax
    call rpn_comm_alltoall(rejectObsSend, nsize, 'MPI_LOGICAL',  &
                           rejectObsRecv, nsize, 'MPI_LOGICAL', 'GRID', ierr)
    call rpn_comm_alltoall(headIndexSend, nsize, 'MPI_INTEGER',  &
                           headIndexRecv, nsize, 'MPI_INTEGER', 'GRID', ierr)
  ...
  subroutine getMyHalo(intInfo, stateVector)
    ...
    integer :: ierr, procIndex, msgIndex, mpiMsgSize, lonIndex, latIndex
    integer :: varLevIndex, numVarLev
    ...
    call rpn_comm_allreduce(intInfo%myHaloSize, myHaloSizeMax, 1,  &
                            'MPI_INTEGER', 'MPI_MAX', 'GRID', ierr)
    ...
    call rpn_comm_alltoall(myHaloSizeMpi,   1, 'MPI_INTEGER',  &
                           yourHaloSizeMpi, 1, 'MPI_INTEGER', 'GRID', ierr)

    ! Maximum size of MPI message for halo
    call rpn_comm_allreduce(maxval(myHaloSizeMpi), mpiMsgSize, 1,  &
                            'MPI_INTEGER', 'MPI_MAX', 'GRID', ierr)
    ...
    call rpn_comm_alltoall(latVecSend, mpiMsgSize, 'MPI_INTEGER',  &
                           latVecRecv, mpiMsgSize, 'MPI_INTEGER', 'GRID', ierr)
    call rpn_comm_alltoall(lonVecSend, mpiMsgSize, 'MPI_INTEGER',  &
                           lonVecRecv, mpiMsgSize, 'MPI_INTEGER', 'GRID', ierr)
  ...
  subroutine getMyInterpObsLatLon(intInfo, stateVector)
    ...
    ! Perform mpi_alltoallv communication
    call mpi_alltoallv(sendObsLat, sendCounts, sendDispls, mmpi_datyp_real8,  &
                       recvObsLat, recvCounts, recvDispls, mmpi_datyp_real8, mmpi_comm_grid, ierr)
    call mpi_alltoallv(sendObsLon, sendCounts, sendDispls, mmpi_datyp_real8,  &
                       recvObsLon, recvCounts, recvDispls, mmpi_datyp_real8, mmpi_comm_grid, ierr)
  ...
  subroutine getMyInterpObsXYposSubGridIndex(intInfo, obsSpaceData, stateVector)
    ...
    ! Perform mpi_alltoallv communication
    call mpi_alltoallv(sendXpos_r4, sendCounts, sendDispls, mmpi_datyp_real4,  &
                       recvXpos_r4, recvCounts, recvDispls, mmpi_datyp_real4,  &
                       mmpi_comm_grid, ierr)
    call mpi_alltoallv(sendYpos_r4, sendCounts, sendDispls, mmpi_datyp_real4,  &
                       recvYpos_r4, recvCounts, recvDispls, mmpi_datyp_real4,  &
                       mmpi_comm_grid, ierr)
    call mpi_alltoallv(sendSubGridIndex, sendCounts, sendDispls, mmpi_datyp_int,  &
                       recvSubGridIndex, recvCounts, recvDispls, mmpi_datyp_int,  &
                       mmpi_comm_grid, ierr)
    call mpi_alltoallv(sendFoot_r4, sendCounts, sendDispls, mmpi_datyp_real4,  &
                       recvFoot_r4, recvCounts, recvDispls, mmpi_datyp_real4,  &
                       mmpi_comm_grid, ierr)
  ...
  subroutine getObsTileMpiId(intInfo, obsSpaceData, stateVector)
    ...
    call rpn_comm_allgather(obsTileMpiId,            numHeaderMaxMpi, 'MPI_INTEGER', &
                            intInfo%allObsTileMpiId, numHeaderMaxMpi, 'MPI_INTEGER', &
                            'GRID', ierr)
  ...
  subroutine getObsLatLon(intInfo, obsSpaceData, stateVector)
    ...
    integer :: numHeader, numVarLev, headerIndex, varLevIndex, ierr
    ...
    call rpn_comm_allgather(numHeader,           1,'mpi_integer', &
                            intInfo%allNumHeader,1,'mpi_integer','grid',ierr)
...
!                        latLev_M, lonLev_M )             ! IN/OUT
  ...
  subroutine sendRecvColumns(myColumnData, column, intInfo, beSilent)
    ...
    integer :: indexBeg, indexEnd, ierr, nsize, procSrc, procDest
    ...
    HeightSfcPresent: if (allocated(myColumnData%GZsfc)) then
      ...
      ! Perform mpi_alltoallv communication for GZsfc
      call mpi_alltoallv(sendBuffer, sendCounts, sendDispls, mmpi_datyp_real8, &
                         recvBuffer, recvCounts, recvDispls, mmpi_datyp_real8, &
                         mmpi_comm_grid, ierr)
    ...
    ! Perform mpi_alltoallv communication for main data
    call mpi_alltoallv(sendBuffer, sendCounts, sendDispls, mmpi_datyp_real8, &
                       recvBuffer, recvCounts, recvDispls, mmpi_datyp_real8, &
                       mmpi_comm_grid, ierr)
  ...
  subroutine sendRecvHalo(stateVector, intInfo, beSilent, &
    ...
    allocate(waitStatusesSend(MPI_STATUS_SIZE,intInfo%yourHaloSize))
    allocate(waitStatusesRecv(MPI_STATUS_SIZE,intInfo%myHaloSize))
    requestIdRecv(:) = 0
    requestIdSend(:) = 0
    waitStatusesRecv(:,:) = 0
    waitStatusesSend(:,:) = 0
  ...
  subroutine sendRecvHaloAd(stateVector, intInfo, beSilent, &
    ...
    allocate(waitStatusesSend(MPI_STATUS_SIZE,intInfo%myHaloSize))
    allocate(waitStatusesRecv(MPI_STATUS_SIZE,intInfo%yourHaloSize))
    requestIdRecv(:) = 0
    requestIdSend(:) = 0
    waitStatusesRecv(:,:) = 0
    waitStatusesSend(:,:) = 0
    ...
    heightSfcPresent: if (stateVector%HeightSfcPresent) then
      ...
      do haloIndex = 1, intInfo%yourHaloSize
        ...
        call mpi_irecv(haloRecvGZsfc(haloIndex),  &
                       nsize, mmpi_datyp_real8, &
                       procSrc, recvTag,  &
                       mmpi_comm_grid, requestIdRecv(numSend), ierr)
      end do
      ...
      do haloIndex = 1, intInfo%myHaloSize
        ...
        call mpi_isend(myHaloValuesAd_r48%GZsfc(haloIndex),  &
                       nsize, mmpi_datyp_real8, &
                       procDest, sendTag,  &
                       mmpi_comm_grid, requestIdSend(numRecv), ierr)
      end do
  ...
  subroutine setupInterpInfo2dFields(interpInfo, obsSpaceData, stateVector,  &
    ...
    call rpn_comm_allreduce(numGridptTotal, numGridptTotalMpi, 1,  &
