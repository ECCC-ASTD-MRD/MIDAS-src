...
module obsFiles_mod
  ...
  public :: obsf_readFiles, obsf_writeFiles, obsf_obsSubRead, obsf_obsSubUpdate
...
contains
  ...
  subroutine obsf_setupFileNames()
    ...
    namePrefix( 1)  = 'brptov'
    namePrefix( 2)  = 'obstov'
    namePrefix( 3)  = 'brpssmis'
    namePrefix( 4)  = 'obsssmis'
    namePrefix( 5)  = 'brpairs'
    namePrefix( 6)  = 'obsairs'
    ...
    namePrefix(14)  = 'obsto_amsub_allsky'
    namePrefix(15)  = 'brpcsr'
    namePrefix(16)  = 'obscsr'
    namePrefix(17)  = 'brpiasi'
    namePrefix(18)  = 'obsiasi'
    namePrefix(19)  = 'brpatms'
    namePrefix(20)  = 'obsatms'
    namePrefix(21)  = 'brpatms_allsky'
    namePrefix(22)  = 'obsatms_allsky'
    namePrefix(23)  = 'brpcris'
    namePrefix(24)  = 'obscris'
    namePrefix(25)  = 'brpcrisfsr'
    namePrefix(26)  = 'obscrisfsr'
    namePrefix(27)  = 'brpcrisfsr1'
    namePrefix(28)  = 'obscrisfsr1'
    namePrefix(29)  = 'brpcrisfsr2'
    namePrefix(30)  = 'obscrisfsr2'
    namePrefix(31)  = 'brpcrisfsr3'
    namePrefix(32)  = 'obscrisfsr3'
    namePrefix(33)  = 'brpcrisfsr4'
    namePrefix(34)  = 'obscrisfsr4'
    namePrefix(35)  = 'obsmwhs2'
    namePrefix(36)  = 'brpmwhs2'
    namePrefix(37)  = 'brpuan'
    namePrefix(38)  = 'brpuas'
    namePrefix(39)  = 'brpai'
    namePrefix(40)  = 'brpain'
    namePrefix(41)  = 'brpais'
    namePrefix(42)  = 'brpaie'
    namePrefix(43)  = 'brpaiw'
    namePrefix(44)  = 'brpsfc'
    namePrefix(45)  = 'brpsf'
    namePrefix(46)  = 'brpsw'
    namePrefix(47)  = 'brpswgoes9'
    namePrefix(48)  = 'brpswgoese'
    namePrefix(49)  = 'brpswgoesw'
    namePrefix(50)  = 'brpswmodis'
    namePrefix(51)  = 'brpswmtsate'
    namePrefix(52)  = 'brpswmtsatw'
    namePrefix(53)  = 'brpgo'
    namePrefix(54)  = 'brpsc'
    namePrefix(55)  = 'brppr'
    namePrefix(56)  = 'brpro'
    namePrefix(57)  = 'brphum'
    namePrefix(58)  = 'brpsat'
    namePrefix(59)  = 'brpssm'
    namePrefix(60)  = 'brpgp'
    namePrefix(61)  = 'brpch'
    namePrefix(62)  = 'brpua'
    ! new general file names for burp and sqlite
    namePrefix(63)  = 'obsua'
    namePrefix(64)  = 'obsuan'
    namePrefix(65)  = 'obsuas'
    namePrefix(66)  = 'obsai'
    namePrefix(67)  = 'obsain'
    namePrefix(68)  = 'obsais'
    namePrefix(69)  = 'obsaie'
    namePrefix(70)  = 'obsaiw'
    namePrefix(71)  = 'obssfc'
    namePrefix(72)  = 'obssf'
    namePrefix(73)  = 'obssw'
    namePrefix(74)  = 'obsswgoes9'
    namePrefix(75)  = 'obsswgoese'
    namePrefix(76)  = 'obsswgoesw'
    namePrefix(77)  = 'obsswmodis'
    namePrefix(78)  = 'obsswmtsate'
    namePrefix(79)  = 'obsswmtsatw'
    namePrefix(80)  = 'obsgo'
    namePrefix(81)  = 'obssc'
    namePrefix(82)  = 'obspr'
    namePrefix(83)  = 'obsro'
    namePrefix(84)  = 'obshum'
    namePrefix(85)  = 'obssat'
    namePrefix(86)  = 'obsssm'
    namePrefix(87)  = 'obsgp'
    namePrefix(88)  = 'obsch'
    namePrefix(89)  = 'obsgl_ssmi'
    namePrefix(90)  = 'obsgl_ssmis'
    namePrefix(91)  = 'obsgl_amsr2'
    namePrefix(92)  = 'obsgl_ascat'
    namePrefix(93)  = 'obsgl_avhrr'
    namePrefix(94)  = 'obsgl_cisA'
    namePrefix(95)  = 'obsgl_cisI'
  ...
  subroutine setObsFilesMpiUniqueList(baseFileNameList,fileTypeList)
    ...
    call mmpi_allGather(baseFileNamelist, baseFileNameListAllMpi, &
                        maxLengthFilename, maxNumObsfiles)
  ...
  ! obsf_obsSubRead
  !--------------------------------------------------------------------------
  function obsf_obsSubRead( obsfam, stnid, varno, nlev, ndim, numColumns_opt, &
    ...
    ! Arguments:
    character(len=*)          , intent(in) :: obsfam  ! observation family name
    character(len=*)          , intent(in) :: stnid   ! station ID of observation
    integer                   , intent(in) :: varno   ! BUFR code (if <=0, to search through all codes to obtain first between 10000 and 16000)
    integer                   , intent(in) :: nlev    ! number of levels in the observation
    integer                   , intent(in) :: ndim    ! number of dimensions for the retrieved data in each report
    integer,          optional, intent(in) :: numColumns_opt ! Number of columns (if different from nlev and for ndim=2; optional)
    integer,          optional, intent(in) :: bkstp_opt      ! bkstp number of requested block if BURP file type (optional)
    integer,          optional, intent(in) :: codtyp_opt(:)  ! Block type of requested block if BURP file type (optional)
                                                             ! Valid values are 'DATA', 'INFO', '3-D', and 'MRQR', indicated
                                                             ! by the two rightmost bits of bknat.
    logical,          optional, intent(in) :: match_nlev_opt ! Determines if the report matching criteria includes checking
                                                             ! if the report number of levels is the same as the input argument nlev (optional)
    character(len=4), optional, intent(in) :: block_opt      ! CODTYP list for search (optional)
    !Result
    ...
    if (fileFound) then
      ...
      write(*,*) "obsf_obsSubRead: No observational files found for family '" // trim(obsfam) // "' for this node."
  ...
  end function obsf_obsSubRead

  !--------------------------------------------------------------------------
  ! obsf_obsSubUpdate
  !--------------------------------------------------------------------------
  function obsf_obsSubUpdate( obsdata, obsfam, varno, bkstp_opt, block_opt, multi_opt ) &
    ...
    ! Arguments:
    type(struct_oss_obsdata),   intent(inout) :: obsdata   ! struct_oss_obsdata object
    character(len=*),           intent(in)    :: obsfam    ! Observation family name
    integer,                    intent(in)    :: varno(:)  ! BUFR code
    integer         , optional, intent(in)    :: bkstp_opt ! bkstp number of requested block if BURP file type (optional)
    character(len=4), optional, intent(in)    :: block_opt ! CODTYP list for search (optional)
    character(len=*), optional, intent(in)    :: multi_opt ! Indicates if report is 'UNI' or 'MULTI' (optional)
    ! Result:
    ...
    if (fileFound) then
       ...
       write(*,*) "obsf_obsSubUpdate: No observational files found for family '" // trim(obsfam) // "' for this node."
    end if
  ...
  end function obsf_obsSubUpdate
