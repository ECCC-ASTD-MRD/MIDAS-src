...
CONTAINS
  ...
  SUBROUTINE adv_setup(adv, mode, hco_in, vco_in, numStepAdvectedField, &
    ...
    call mmpi_allGather(adv%myLatBeg,  adv%allLatBeg, communicator_opt = mmpi_comm_NS)
  ...
  SUBROUTINE adv_statevector_tl( statevector, adv)
    ...
    call rti_tmg_start(100,'--ADV_GSV')
    ...
    do varLevIndex = 1, gsv_getNumVarLev(statevector)
      ...
      do stepIndexAF = 1, adv%nTimeStep
        ...
        if (.not. gatheringDone ) then
          ...
          ! gather the global field to be interpolated on all tasks
          call mmpi_barrier
          call rti_tmg_start(101,'----ADV_GSV_Comm')
          ...
          call rti_tmg_stop(101)

          ! rearrange gathered fields for convenience
          call rti_tmg_start(102,'----ADV_GSV_Shuffling')
          ...
          !$OMP END PARALLEL DO
          call rti_tmg_stop(102)

          if (adv%singleTimeStepIndexSource) gatheringDone = .true.

        end if

        call rti_tmg_start(103,'----ADV_GSV_Calc')
