...
module bgckMicrowave_mod
  ...
  logical :: mwbg_useClwDiffForAllskyBcorr
  ...
  logical            :: useClwDiffForAllskyBcorr
  ...
  namelist /nambgck/instName, clwQcThreshold, &
                    ...
                    cloudyClwThresholdBcorr, clwDiffThreshBcorr, modLSQ, &
...
contains
  ...
  !--------------------------------------------------------------------------
  subroutine amsuABTest2LandSeaQualifierCheck(sensorIndex, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsuABTest3TerrainTypeCheck(sensorIndex, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsuABTest4FieldOfViewCheck(sensorIndex, maxScanAngleAMSU, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsuABTest5ZenithAngleCheck(sensorIndex, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsuABTest11RadianceGrossValueCheck(sensorIndex, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsuaTest12GrodyClwCheck(sensorIndex, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsuaTest13GrodyScatteringIndexCheck(sensorIndex, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsuaTest14RogueCheck(sensorIndex, headerIndex, obsSpaceData)
    !
  ...
  !--------------------------------------------------------------------------
  subroutine amsubTest14RogueCheck(sensorIndex, headerIndex, obsSpaceData, &
                                   skipTestArr_opt)
    !
  ...
  subroutine amsuABTest15ChannelSelectionWithTovutil(sensorIndex, headerIndex, obsSpaceData)
    ...
    !          We set QC flag bits 7 and 9 ON for channels with oer_tovutil=2
    !          over land or sea-ice.
    !
    !          We set QC flag bits 7 and 9 ON for channels
    ...
    BODY: do bodyIndex = bodyIndexBeg, bodyIndexEnd
      ...
      if ( mwbg_debug ) write(*,*) 'mwbg_qcIndicator = ', mwbg_qcIndicator(obsChanNum)
    end do BODY
  ...
  !--------------------------------------------------------------------------
  subroutine amsuaTest16ExcludeExtremeScattering(sensorIndex, headerIndex, obsSpaceData)
    !
    ...
    ! Locals:
    integer :: testIndex, INDXCAN, landQualifierIndice, bodyIndex, bodyIndexBeg, bodyIndexEnd
    integer :: obsChanNum, obsChanNumWithOffset, codtyp
  ...
  subroutine mwbg_tovCheckAmsua(sensorIndex, headerIndex, obsSpaceData)
    ...
    if (mwbg_resetQc) then
      BODY: do bodyIndex = bodyIndexBeg, bodyIndexEnd
        call flg_resetFlag(obsSpaceData, bodyIndex)
      end do BODY
    end if
    ...
    ! For Cloud Liquid Water > clwQcThreshold, reject AMSUA-A channels 1-5 and 15.
    call amsuaTest12GrodyClwCheck(sensorIndex, headerIndex, obsSpaceData)
  ...
  subroutine mwbg_tovCheckAmsub(sensorIndex, headerIndex, obsSpaceData)
    ...
    ! Les observations, dont le residu (O-P) depasse par un facteur (mwbg_rogueFactor) l'erreur totale des TOVS.
    ! N.B.: a reject by any of the 3 surface channels produces the rejection of AMSUA-A channels 1-5 and 15.
    call amsubTest14RogueCheck(sensorIndex, headerIndex, obsSpaceData, &
                               skipTestArr_opt=skipTestArr(:))
  ...
  ! mwbg_qcStats
  !--------------------------------------------------------------------------
  subroutine mwbg_qcStats(instName, sensorIndex, satelliteId, LDprint)
    !
  ...
  subroutine atmsMwhs2Test1Flagbit7Check(sensorIndex, headerIndex, obsSpaceData, &
    ...
    if (present(skipTestArr_opt)) then
      if (skipTestArr_opt(testIndex)) then
        if (firstCall) then
          firstCall = .false.
          write(*,*) 'atmsMwhs2Test1Flagbit7Check: skipping this test'
        end if
        return
      end if
    end if
  ...
  subroutine atmsMwhs2Test2TopographyCheck(sensorIndex, headerIndex, obsSpaceData, &
    ...
    if (present(skipTestArr_opt)) then
      if (skipTestArr_opt(testIndex)) then
        if (firstCall) then
          firstCall = .false.
          write(*,*) 'atmsMwhs2Test2TopographyCheck: skipping this test'
        end if
        return
      end if
    end if
  ...
  subroutine atmsMwhs2Test3UncorrectedTbCheck(sensorIndex, headerIndex, obsSpaceData, &
    ...
    if (present(skipTestArr_opt)) then
      if (skipTestArr_opt(testIndex)) then
        if (firstCall) then
          firstCall = .false.
          write(*,*) 'atmsMwhs2Test3UncorrectedTbCheck: skipping this test'
        end if
        return
      end if
    end if
  ...
  subroutine atmsTest4RogueCheck(sensorIndex, waterobs, headerIndex, obsSpaceData, &
    ...
    if (present(skipTestArr_opt)) then
      if (skipTestArr_opt(testIndex)) then
        if (firstCall) then
          firstCall = .false.
          write(*,*) 'atmsTest4RogueCheck: skipping this test'
        end if
        return
      end if
    end if
    ...
    if (SFCREJCT) then
      BODY2: do bodyIndex = bodyIndexBeg, bodyIndexEnd
        ...
        if ( INDXCAN /= 0 ) then
          ...
          end if ! if ( mwbg_qcIndicator(obsChanNum)
        end if ! if ( INDXCAN /= 0 )
      end do BODY2
    end if ! SFCREJCT
  ...
  subroutine Mwhs2Test4RogueCheck(sensorIndex, waterobs, headerIndex, obsSpaceData, &
    ...
    if (present(skipTestArr_opt)) then
      if (skipTestArr_opt(testIndex)) then
        if (firstCall) then
          firstCall = .false.
          write(*,*) 'Mwhs2Test4RogueCheck: skipping this test'
        end if
        return
      end if
    end if
  ...
  !--------------------------------------------------------------------------
  subroutine atmsMwhs2Test5ChannelSelectionUsingTovutil(sensorIndex, headerIndex, obsSpaceData, &
                                                        skipTestArr_opt)
    !
    ...
    if (present(skipTestArr_opt)) then
      if (skipTestArr_opt(testIndex)) then
        if (firstCall) then
          firstCall = .false.
          write(*,*) 'atmsMwhs2Test5ChannelSelectionUsingTovutil: skipping this test'
        end if
        return
      end if
    end if
  ...
  subroutine mwbg_tovCheckAtms(sensorIndex, headerIndex, obsSpaceData)
    ...
    integer :: bodyIndex, bodyIndexBeg, bodyIndexEnd, codtyp
    ...
    if (firstCall) then
      ...
      allocate(mwbg_chanRejectForTopoFilter(5))
    ...
    !###############################################################################
    call mwbg_flagDataUsingNrlCritAtms(scatIndexOverWaterObsClearsky, scatIndexOverWaterObsEcmwf, &
    ...
    !  oer_tovutil = 0 (blacklisted)
    !                1 (assmilate)
    call atmsMwhs2Test5ChannelSelectionUsingTovutil(sensorIndex, headerIndex, obsSpaceData, &
                                                    skipTestArr_opt=skipTestArr(:))
  ...
  subroutine mwbg_tovCheckMwhs2(sensorIndex, lastHeader, headerIndex, obsSpaceData)
    ...
    logical, allocatable :: qcRejectLogic(:)
    real(8) :: zdi, scatIndexOverWaterObsEcmwf, scatIndexObsClearsky, SeaIce, riwv
    logical, save :: firstCall = .true.
    integer, save :: numReportWithMissingTb
    ...
    !###############################################################################
    ! STEP 4 ) mwbg_nrlFilterMwhs2 returns scatIndexOverWaterObsEcmwf, scatIndexObsClearsky and also does sea-ice
    !          detection missing value for cloudLiquidWaterPathObs, scatIndexOverWaterObsEcmwf, scatIndexObsClearsky is mwbg_realMissing (e.g. over
    !          land or sea-ice).
    !###############################################################################
    call mwbg_nrlFilterMwhs2(calcLandQualifierIndice, waterobs, grossrej, &
                             scatIndexObsClearsky,scatIndexOverWaterObsEcmwf, &
    ...
    !###############################################################################
    call mwbg_reviewAllCritforFinalFlagsMwhs2(qcRejectLogic, grossrej, calcTerrainTypeIndice, waterobs, precipobs, &
                                              scatIndexObsClearsky, scatIndexOverWaterObsEcmwf, &
                                              iwvreject, riwv, &
    ...
    if (lastHeader) then
      ...
      write(*,*) '       4     scatIndexOverWaterObsEcmwf > Lower Troposphere limit=9 (precipobs)'
      ...
      write(*,*) '       8     scatIndexObsClearsky > CMC amsu-b limit (land=0,sea=15,ice=40)'
  ...
  subroutine mwbg_nrlFilterAtms(calcLandQualifierIndice, calcTerrainTypeIndice, waterobs, grossrej, &
    ...
    if (.not. grossrej) then
      ...
      allocate(btClear(actualNumChannel))
    ...
    scatIndexOverWaterObs      = mwbg_realMissing
    scatIndexOverWaterFG       = mwbg_realMissing
    scatIndexOverWaterObsEcmwf = mwbg_realMissing
    scatIndexOverWaterObsClearsky = mwbg_realMissing
    SeaIce = 0.0d0
    ...
    tb165FG = mwbg_realMissing
    bcor165 = mwbg_realMissing
    tb89FgClear  = mwbg_realMissing
    tb165FgClear = mwbg_realMissing
    ...
    if (grossrej) then
      ...
      ! 165 Ghz = AMSU-B 2 = ATMS channel 17
      tb23    = obsTb(1)
    ...
    if ( ier == 0 ) then
      ...
      if (waterobs) then
        ...
        if (instrumentIsAllskyHu) then
          ...
          scatIndexOverWaterObs = (tb89 - tb165) - (-39.2010d0 + 0.1104d0 * satZenithAngle)
        end if

        scatIndexOverWaterObsClearsky = (tb89 - tb165) - (-39.2010d0 + 0.1104d0 * satZenithAngle)
        scatIndexOverWaterObsEcmwf = (tb89 - tb165) - (-46.94d0 + 0.248d0 * satZenithAngle)
      end if
    ...
    if ( mwbg_debug ) then
      ...
      write(*,*) ' ier,ice,cloudLiquidWaterPathObs,cloudLiquidWaterPathFG,scatIndexOverWaterObsEcmwf,scatIndexOverWaterObsClearsky,scatIndexOverWaterObs,calcTerrainTypeIndice,waterobs =', &
                 ier,ice,cloudLiquidWaterPathObs,cloudLiquidWaterPathFG,scatIndexOverWaterObsEcmwf,scatIndexOverWaterObsClearsky,scatIndexOverWaterObs,calcTerrainTypeIndice,waterobs
    end if
  ...
  subroutine mwbg_nrlFilterMwhs2(calcLandQualifierIndice, waterobs, grossrej, &
    ...
    !
    !          NOTES:
    !            - <scatIndexOverWaterObsEcmwf> only computed over open water away from coasts and sea-ice
    !            - <scatIndexObsClearsky> is computed for all points
    !          REFERENCES: Ben Ruston, NRL Monterey
    !          JCSDA Seminar 12/12/12: Impact of NPP Satellite Assimilation in the U.S. Navy Global Modeling System
    !
    !          Notes: In the case where an output parameter cannot be calculated, the
    ...
    logical,          intent(in)    :: grossrej     ! .true. if any channel had a gross error from mwbg_grossValueCheck
    logical,          intent(inout) :: waterobs     ! .true. if open water point (away from coasts and sea-ice)
    real(8),          intent(out)   :: scatIndexObsClearsky  ! Bennartz-Grody scattering index from tb89 & tb165
    ...
    scatIndexOverWaterFG       = mwbg_realMissing
    scatIndexOverWaterObsEcmwf = mwbg_realMissing
    scatIndexObsClearsky = mwbg_realMissing
    SeaIce   = 0.0d0
    ...
    if ( ier == 0 ) then
      ...
      if ( waterobs ) then
        ...
        scatIndexObsClearsky = (tb89 - tb165) - (-39.201d0 + 0.1104d0 * satZenithAngle)
        scatIndexOverWaterObsEcmwf = (tb89 - tb165) - (-46.94d0 + 0.248d0 * satZenithAngle)
      else
        scatIndexObsClearsky = (tb89 - tb165) - (0.158d0 + 0.0163d0 * satZenithAngle)
      end if
    ...
    if ( mwbg_debug ) then
      ...
      write(*,*) 'ier,ice,cloudLiquidWaterPathObs,cloudLiquidWaterPathFG,scatIndexOverWaterObsEcmwf,scatIndexObsClearsky,waterobs =', &
                  ier,ice,cloudLiquidWaterPathObs,cloudLiquidWaterPathFG,scatIndexOverWaterObsEcmwf,scatIndexObsClearsky,waterobs
    end if
  ...
  !--------------------------------------------------------------------------
  subroutine mwbg_flagDataUsingNrlCritAtms(scatIndexOverWaterObsClearsky, scatIndexOverWaterObsEcmwf, &
                                           SeaIce, grossrej, waterobs, &
                                           iwvreject, cloudobs, precipobs,  cldcnt, riwv, zdi, &
                                           headerIndex, sensorIndex, obsSpaceData)
    !
    ...
    ! Arguments:
    real(8),          intent(in)    :: scatIndexOverWaterObsClearsky ! clear-sky scattering index from tb89 & tb165
  ...
  subroutine mwbg_flagDataUsingNrlCritMwhs2(scatIndexOverWaterObsEcmwf, &
    ...
    !            - 4     scatIndexOverWaterObsEcmwf > Lower Troposphere limit 9/10 (precipobs)
    !            - 5     Mean 183 Ghz [ch. 18-22] Tb < 240K
  ...
  subroutine mwbg_reviewAllCritforFinalFlagsAtms(qcRejectLogic, grossrej, waterobs, precipobs, &
    ...
    real(8) :: scatIndexOverWaterObsUsed, clwDiff
    ...
    if (grossrej) then
      ...
      if  (.not. waterobs) then
        ...
        !   -- reject ch. 16-22      if scatIndexOverWaterObsEcmwf > 9 and
    ...
    if (precipobs .and. waterobs) pcpcnt = pcpcnt + 1
  ...
  ! mwbg_reviewAllCritforFinalFlagsMwhs2
  !--------------------------------------------------------------------------
  subroutine mwbg_reviewAllCritforFinalFlagsMwhs2(qcRejectLogic, grossrej, calcTerrainTypeIndice, waterobs, precipobs, &
                                                  scatIndexObsClearsky, scatIndexOverWaterObsEcmwf, &
                                                  iwvreject, riwv, &
                                                  zdi, allcnt, drycnt, landcnt, &
                                                  rejcnt, iwvcnt, pcpcnt, flgcnt, &
    ...
    ! Arguments:
    logical,          intent(in)    :: qcRejectLogic(:) ! .true. if channel is rejected
    real(8),          intent(in)    :: scatIndexObsClearsky ! Bennartz-Grody scattering index from tb89 & tb165
    real(8),          intent(in)    :: scatIndexOverWaterObsEcmwf    ! ECMWF scattering index from tb89 & tb165
    logical,          intent(in)    :: grossrej         ! .true. if any channel had a gross error from mwbg_grossValueCheck
    logical,          intent(in)    :: waterobs         ! if obs over open-water
    logical,          intent(in)    :: iwvreject        ! .true. if Mean 183 Ghz [ch. 18-22] Tb < 240K (too dry for ch.20-22 over land)
    ...
    integer :: bodyIndex, bodyIndexBeg, bodyIndexEnd, obsChanNum, obsChanNumWithOffset
    real(8) :: cloudLiquidWaterPathObs, cloudLiquidWaterPathFG
    real(8) :: scatbg_rej, scatIndexOverWaterObs, scatIndexOverWaterFG, scatwObsFGaveraged
    ...
    if (grossrej) then
      ...
      !    -- check DI for AMSU-B like channels
      !    -- reject all channels if scatIndexObsClearsky exceeds CMC thresholds for AMSU-B
      ...
      if  (.not. waterobs) then
        ...
        if (scatIndexObsClearsky > scatbg_rej) then
          lflagchn(:) = .true.
      ...
      !    -- reject ch. 1, 10-15 if scatIndexOverWaterObsEcmwf > 9  or scatIndexOverWaterObsEcmwf = mwbg_realMissing
      !    -- reject ch. 1, 10-15 if iwvreject = .true.   [ Mean 183 Ghz [ch. 11-15] Tb < 240K ]
      !    -- reject all channels if scatIndexObsClearsky exceeds CMC SEA threshold for AMSU-B

        if (cloudLiquidWaterPathObs > clw_mwhs2_nrl_LTrej) then
          lflagchn((/1,10/)) = .true.

          if (.not. instrumentIsAllskyHu .or. mwbg_allowClwRejectHuChanAllskyHu) then
            lflagchn((/13,14,15/)) = .true.
          end if
        end if

        if (cloudLiquidWaterPathObs > clw_mwhs2_nrl_UTrej)  then
          if (.not. instrumentIsAllskyHu .or. mwbg_allowClwRejectHuChanAllskyHu) lflagchn(11:12) = .true.
        end if
        ...
        if (instrumentIsAllskyHu) then
          ...
          if (scatIndexOverWaterObsEcmwf >  scatec_mwhs2_nrl_LTrej) then
            lflagchn(1) = .true.
            lflagchn(10:15) = .true.
          end if
        end if
        ...
        ! Bennartz-Grody SI check thresholds (same as for QC of AMSU-B/MHS)
        if (instrumentIsAllskyHu .and. .not. mwbg_useScatIndexOverWaterObsClearsky) then
          scatIndexOverWaterObsUsed = scatIndexOverWaterObs
        else
          scatIndexOverWaterObsUsed = scatIndexObsClearsky
        end if
        if (scatIndexOverWaterObsUsed > scatbg_mwhs2_cmc_SEA) then
          lflagchn(:) = .true.
          newInformationFlag = IBSET(newInformationFlag,8)
        end if
      end if  ! if waterobs
