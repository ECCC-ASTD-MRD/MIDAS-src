  ...
  subroutine csg_toolbox
    ...
    do waveBandIndex = 1, nWaveBand
       ...
       if (trim(tool) == 'HVCORREL_HI') then
          ...
          call calcLocalizationRadii(ensPerturbations, stddev3d, variableType, waveBandIndex=waveBandIndex) ! IN
       end if
...
!$OMP END PARALLEL DO
  ...
  !--------------------------------------------------------------------------
  subroutine calcLocalizationRadii(ensPerturbations,ensStdDev,variableType,waveBandIndex)
    implicit none
...
!$OMP PARALLEL DO PRIVATE (k,iref,jref,j,i,ens,correlation,covariance, &
!$OMP                      fourthMoment,distance,bin,weight,ensPert_local)
    do k = 1, nkgdimEns
...
!!$                               (ensPerturbations(i,j,k,ens) * ensPerturbations(iref,jref,k,ens))**2
!!$                     end if
!!$                   end do
!!$                end do
!!$             end do
