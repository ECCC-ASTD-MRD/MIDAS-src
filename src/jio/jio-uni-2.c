...
static void jio_trace(long long nsec, long long jstmp, const char * cc, int file, int len, const char *name) 
{
  ...
  buflen=snprintf(buffer,sizeof(buffer),"JIO%s Trace ",name!=NULL?"2":"1");  /* JIO1 if no filename, JIO2 if there is one */
  buflen+=snprintf(buffer+buflen,sizeof(buffer),"%s thrd=%2d, file =%3d, bytes =%10d",cc,it,file,len);
  buflen+=snprintf(buffer+buflen,sizeof(buffer),", time =%12Ld nsec",nsec);
  buflen+=snprintf(buffer+buflen,sizeof(buffer),", stamp=%20Ld",jstmp);
  if(name != NULL) buflen+=snprintf(buffer+buflen,sizeof(buffer),", file= %s",name);
  buflen+=snprintf(buffer+buflen,sizeof(buffer),"\n");
