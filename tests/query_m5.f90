...
!    Les enregistrments dans une matrice integer8
program query_m2
   use fSQLite
   ...
   ! matrice de type integer pour sauver le resulat de la requette
   integer*8 , dimension(:,:), pointer :: matrix_int8
   ...
   CALL fSQL_get_many(stmt,NROWS = n,NCOLS = m, MODE = FSQL_INT8, INT8_MISSING= INT(-99,INTEGER8_KIND_CONST)) 

   write(*,*) 'INTEGER8_KIND_CONST = ', INTEGER8_KIND_CONST

   write (*,*) 'Enregestriments trouves = ',n, 'de ', m, 'Colonnes'
   

   ! Allouer la matrice avec les bonnes dimensions
   allocate(matrix_int8(n,m)); 
   

   ! Mettre les resultats dans la matrice
   CALL fSQL_fill_matrix(stmt,matrix_int8) 
   

   ! Sortie des resultats
   write( *, '(5a10)' ) 'ID', 'CANAL', 'BT','O_A','O_P'
   CALL fSQL_write_matrix(matrix_int8) 
   ...
   allocate(IDs(n))
   IDs = INT(matrix_int8(:,1), KIND = INTEGER4_KIND_CONST) 
   ...
   allocate(CNs(n))
   CNs = INT(matrix_int8(:,2), KIND = INTEGER4_KIND_CONST) 
