...
!    Les enregistrments dans une matrice real8
program query_m2
   use fSQLite
   ...
   real*8 , dimension(:,:), pointer :: matrix_real8
   ...
   CALL fSQL_get_many(stmt,NROWS = n,NCOLS = m, MODE = FSQL_REAL8, REAL8_MISSING = REAL(-99.0,8)) 

   write (*,*) 'Enregestriments trouves = ',n, 'de ', m, 'Colonnes'
   
   !! Allouer la matrice avec les bonnes dimensions
   allocate(matrix_real8(n,m)); 
   

   !! Mettre les resultats dans la matrice
   CALL fSQL_fill_matrix(stmt,matrix_real8) 
   

   !! Sortie des resultats
   write( *, '(5a10)' ) 'ID', 'CANAL', 'BT','O_A','O_P'
   CALL fSQL_write_matrix(matrix_real8) 
   ...
   allocate(IDs(n))
   IDs = INT(matrix_real8(:,1), KIND = INTEGER4_KIND_CONST) 
   ...
   allocate(CNs(n))
   CNs = INT(matrix_real8(:,2), KIND = INTEGER4_KIND_CONST) 
