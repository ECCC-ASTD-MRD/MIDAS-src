...
!    Les enregistrments dans une matrice integer*4 
program query_m4
   use fSQLite
   ...
   integer*4 , dimension(:,:), pointer :: matrix_int
   ...
   CALL fSQL_get_many(stmt,NROWS = n,NCOLS = m, MODE = FSQL_INT, INT_MISSING = 1111) 

   write (*,*) 'Enregestriments trouves = ',n, 'de ', m, 'Colonnes'
   
   ! Allouer la matrice avec les bonnes dimensions
   allocate(matrix_int(n,m)); 

   ! Mettre les resultats dans la matrice
   CALL fSQL_fill_matrix(stmt,matrix_int) 

   ! Sortie des resultats
   write( *, '(5a10)' ) 'ID', 'CANAL', 'BT','O_A','O_P'
   CALL fSQL_write_matrix(matrix_int) 
