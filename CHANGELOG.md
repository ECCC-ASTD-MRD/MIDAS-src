 ...
 * Added ability to update SQLite columns if they doesn't previously exist (#601 and !630)
 ...
 * Using `oavar_scripts` to version `2.2.7` which includes the following changes since version `2.2.6`:
   ...
   * La variable `OAVAR_OBS_MPI_ORDERING` est mise a `regular` par
     d�faut
      - Cela est coh�rent avec le code de MIDAS depuis la version
       `v_3.5.0`.
 ...
 * Using `oavar_scripts` to version `2.2.6` which includes the following changes since version `2.2.4`:
       ...
       * `oavar.var_mpi`: On corrige le mode `fasttmp=no` pour �viter que les
         fichiers complets se retrouvent dans le meme r�pertoire que les
         fichiers splitt�s.
