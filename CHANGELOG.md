 ...
 * Adding functionality to `prepcma` to thin radiance obs independently with different max number of headers (#961 and !873)
   * New namelist variables `thinTovsPerInst`, `tovsInstNamesWithMaxNumHeaders`, and `maxNumHeadersForTovsInst` added to `namprepcma` namelist.
   * Minor impact on results if this option is activated.
