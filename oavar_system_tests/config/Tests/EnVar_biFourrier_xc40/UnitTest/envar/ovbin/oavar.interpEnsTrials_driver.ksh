...
export INTERPENSTRIALS_VGRID_PACKAGE=${INTERPENSTRIALS_VGRID_PACKAGE:-eccc/cmd/cmdn/utils/201611/04/default}
export INTERPENSTRIALS_CMDN_PACKAGE=${INTERPENSTRIALS_CMDN_PACKAGE:-eccc/cmd/cmdn/pxs2pxt/3.7.7/default}
...
if [ -z "${INTERPENSTRIALS_WRITE_SUBDOMAINS_PROGRAM}" ]; then
    ...
    INTERPENSTRIALS_WRITE_SUBDOMAINS_PROGRAM=$(which write_subdomains.Abs)
fi
...
${touch_cmd} abort
status=0
time $(which oavar.interpEnsTrials.ksh) ${prefix_machine}${jfile} ${proto_file} ${WORKDIR} $(basename ${jfile})
status=\$?
[ "\${status}" -eq 0 ] && ${rm_cmd} abort
...
if [ -d ${WORKDIR}/workdir_\${job} ]; then
    RAMTMPDIR=${ramdiskpath}/interpEnsTrials_\${MP_CHILD}_\$\$
...
EOF
fi  ## Fin du 'else' relie au 'if [ -n "${ramdiskpath}" ]'
...
mkdir ${LOCAL_BIN}
ln -s $(which cclargs) $(which r.fstinfo) ${LOCAL_BIN}
...
while [ "${base}" -lt "${index}" ]; do
    ...
    abort=0
    ${INTERPENSTRIALS_MPIRUN} "Lauching interpolation with $(which oavar.interpEnsTrials.ksh)" "${mpi_ssm}" ${omp_stacksize} ${TASK_BIN}/r.run_in_parallel -pgm ./task.sh -npex ${num_procs} -nompi pseudo_mpi -extrapath ${LOCAL_BIN} -tmpdir ${PWD}/interp_mpi_tmpdir || abort=1

    job=${base}
    while [ "${job}" -lt "$((base+num_procs))" ]; do
	if [ -d ${WORKDIR}/workdir_${job} ]; then
	    number=$((job+1))
	    if [ -f ${WORKDIR}/workdir_${job}/abort ]; then
